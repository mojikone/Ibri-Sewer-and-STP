...
def write_all(out_dir, net: Network, riders, still_low, pockets, boundary, of_rep,
    ...
    gpd.GeoDataFrame({
        ...
        os.path.join(out_dir, "W8_manholes.shp"), encoding="utf-8")
    ...
    gpd.GeoDataFrame({
        ...
        "TIER": [r.tier for r in R],                    # trunk main / sub main / lateral
        ...
        os.path.join(out_dir, "W8_pipes.shp"), encoding="utf-8")
    ...
    if riders:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_tertiary_riders.shp"), encoding="utf-8")
    if spurs:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_tertiary_connections.shp"), encoding="utf-8")
    if stubs:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_tertiary_stubouts.shp"), encoding="utf-8")
    ...
    if still_low:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_lowplots.shp"), encoding="utf-8")
    ...
    if catchments:
        ...
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_catchments.shp"), encoding="utf-8")
    ...
    if trunk_wkt:
        ...
        gpd.GeoDataFrame({"PART": list(range(1, len(gs) + 1)),
            ...
            os.path.join(out_dir, "W8_main_pipe.shp"), encoding="utf-8")

    # chambers that could not be freed from a plot: the road centreline itself runs
    # through the plot, so this is a question for the road or cadastre data
    from shapely.strtree import STRtree as _ST
    if getattr(crit, "_all_plots", None):
        pass
    ...
    if sharp:
        ...
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_sharp_inlets.shp"), encoding="utf-8")
    ...
    if stations:
        ...
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W8_pumping_stations.shp"), encoding="utf-8")
    ...
    if pockets:
        gpd.GeoDataFrame({
            ...
            crs=CRS).to_file(os.path.join(out_dir, "W8_sls_candidates.shp"), encoding="utf-8")
    ...
    gpd.GeoDataFrame({"LABEL": ["OF-1"], "GND": [round(of_rep["z"], 3)],
        ...
        os.path.join(out_dir, "W8_outfall.shp"), encoding="utf-8")

    gpd.GeoDataFrame({"NAME": ["test boundary (repaired)"]}, geometry=[boundary],
                     crs=CRS).to_file(os.path.join(out_dir, "W8_boundary.shp"),
                                      encoding="utf-8")
