...
def main():
    ...
    X.write_dxf(os.path.join(cfg.OUT_DXF, "W14_A_ground.dxf"), runs, run_catch, info, polys,
    ...
    X.write_shapes(cfg.OUT_SHP, "W14_A", runs, run_catch, info, polys, nodes, streams, cfg.EPSG)
    ...
    # 3b. (W14 trial, after the spines, so a pump pocket never swallows a settlement that has a trunk, the engineer's line of 2026-09-08 in rule 2's terms) a basin is a
    #     pocket when its fill plus the fall from its low to its rim cannot fit in 12 m: no
    #     street may carry it out by gravity, so no street is asked to
    if getattr(cfg, "POCKET_BY_DEPTH", False):
        edge_ = {}
        for r_ in runs:
            edge_[(r_["up"], r_["dn"])] = r_["len"]
            edge_[(r_["dn"], r_["up"])] = r_["len"]
        for _ in range(40):
            new_pockets = []
            for s_ in raw_sinks:
                if s_ in targets2 or s_ not in filled2:
                    continue
                fill = filled2[s_] - znode[s_]
                if fill <= cfg.HOLLOW_M:
                    continue
                # the fall at the minimum gradient from the low to where the basin ends
                m, L_ = s_, 0.0
                while m in parent2 and parent2[m] is not None and filled2.get(m, 0) > znode.get(m, 0) + 0.05:
                    q_ = parent2[m]
                    L_ += edge_.get((m, q_), 0.0)
                    m = q_
                need = fill + cfg.SMIN_PROXY * L_ + cfg.TRUNK_COVER_M
                if need > cfg.MAX_DEPTH_M:
                    new_pockets.append((s_, round(fill, 1), round(L_), round(need, 1)))
            if not new_pockets:
                break
            # one pocket a round, the worst first: its neighbours drain to it next round
            # instead of each becoming a pump of its own
            new_pockets = [max(new_pockets, key=lambda t_: t_[3])]
            for s_, fill, L_, need in new_pockets:
                targets2[s_] = "SINK"
                log(f"   pocket by depth at {tuple(round(v) for v in s_)}: fill {fill} m, {L_} m to the rim, "
                    f"{need} m at the rim against {cfg.MAX_DEPTH_M}")
            src2, filled2, parent2, seq2, spill2, iters2 = O.resolve_outlets(
                runs, znode, targets2, cfg.BASIN_MAX_M, raw_term, **assign)
        rep["pockets_by_depth"] = sum(1 for t_ in targets2.values() if t_ == "SINK")
    ...
    for rr in range(cfg.REROUTE_ROUNDS + 2):
        ...
        if getattr(cfg, "SKELETON_FIRST", False):
            # W14 trial: sub-mains first, catchments follow the skeleton (rule 4 as written)
            from sewnet import skeleton_first as SF
            owner0 = {t: t for t in targets2}
            for n in trunk_stem:                  # a trunk node drains to its trunk's end
                m, seen_ = n, set()
                while m in trunk_stem and m not in seen_:
                    seen_.add(m)
                    m = trunk_stem[m]
                owner0[n] = owner0.get(m, m)
            stem_parent, submain, owner, left, srep3 = SF.assemble(
                runs, znode, owner0, chains, filled2, cfg.LEVEL_M, cfg.CHAIN_MIN_M,
                cfg.SKEL_LINK_M, cfg.DEPTH_WEIGHT, cfg.SMIN_PROXY,
                max_depth=cfg.MAX_DEPTH_M, cover=cfg.TRUNK_COVER_M)
            for pk in srep3.pop("pockets", []):
        ...
        par, dist, unreached = K.build_tree(runs, znode, submain, stem_parent, cfg.DEPTH_WEIGHT,
                                            ...
                                            src=src_filter, submain_discount=cfg.SUBMAIN_DISCOUNT)
    ...
    try:
        dxf = XT.write_dxf(os.path.join(cfg.OUT_DXF, "W14_A_tree.dxf"), pipes, gaps, dropped_geoms,
        ...
        dxf = XT.write_dxf(os.path.join(cfg.OUT_DXF, "W14_A_tree_new.dxf"), pipes, gaps,
    ...
    XT.write_shapes(cfg.OUT_SHP, "W14_A_tree", pipes, gaps, info2, polys2, {**joins, **link_geoms},
                    cfg.EPSG)
    b = envelope.bounds
    XT.write_png(os.path.join(cfg.OUT_IMG, "W14_A_tree_overview.png"),
    ...
    XT.write_png(os.path.join(cfg.OUT_IMG, "W14_A_tree_east.png"), EAST, pipes, gaps, info2,
                 polys2, colour2, joins, streams, main_pipe, built_geoms, envelope, cfg.STP,
                 "Stage A tree, east settlement")
    XT.write_png(os.path.join(cfg.OUT_IMG, "W14_A_tree_west.png"), WEST, pipes, gaps, info2,
                 polys2, colour2, joins, streams, main_pipe, built_geoms, envelope, cfg.STP,
