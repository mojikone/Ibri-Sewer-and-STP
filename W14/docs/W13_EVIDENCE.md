...
## W14 trial: sub-mains first, and the maze is gone (2026-09-08)

The engineer's reading of W13's drawing: NAMA's layout is easy to build, a contractor sees
which road is main; ours was a maze, every street a guess, sub-networks nested into each
other. Cause found: W13 chose every node's outlet first and picked sub-mains per outlet
afterwards, the reverse of rule 4. W14 is a copy of W13's engine with one switch,
`SKELETON_FIRST`: the long straight streets are attached to an outlet or to a bigger street
first, longest first, a whole street choosing its outlet; every other street drains to the
nearest skeleton node; the sub-networks follow the skeleton. Same ground, same targets,
48 seconds. Kept beside it, not in the logic: the engineer's line on basins as a pocket test
(`POCKET_BY_DEPTH`): a basin is a pocket when its fill plus the fall at 0.5 % from its low
to its rim, plus cover, passes 12 m; one pocket a round, the worst first, so a basin system
becomes one pump and not twenty; run after the spines, so a pump pocket never swallows a
settlement that has a trunk to the works.

| | W13, catchments first | W14, sub-mains first |
|---|---|---|
| Pieces per sub-network | many, interleaved | **one, every sub-network a single block** |
| Long streets attached | | 178 of 262, 12 km of connector, 5 left as laterals |
| Sub-main share | 30 to 43 % | 40 % (the 250 m floor is the engineer's to tune) |
| Sub-networks | 35 | 44: 35 joins, 3 to the works (63 km), **4 pumps (25 km)**, 2 islands |
| Pumps | 1 | the low ground west of the settlement, the basin system south of it, the low by the ridge, the low ground south-west of the works |
| Chambers over 12 m | 27 | 29 |
| Deepest | 16.1 m | 17.5 m |
| At the works | 7.8 m under | 8.7 m under |

Where the four remaining failures are: two streets climbing to joins on the ridge (17.5 and
12.7 m, the "street rising to the join" case the engineer said to cut where its halves part);
