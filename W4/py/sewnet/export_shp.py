...
def write_all(out_dir, net: Network, riders, still_low, pockets, boundary, of_rep,
    ...
    gpd.GeoDataFrame({
        ...
        os.path.join(out_dir, "W4_manholes.shp"), encoding="utf-8")
    ...
    gpd.GeoDataFrame({
        ...
        os.path.join(out_dir, "W4_pipes.shp"), encoding="utf-8")
    ...
    if riders:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W4_riders.shp"), encoding="utf-8")
    ...
    if still_low:
        gpd.GeoDataFrame({
            ...
            os.path.join(out_dir, "W4_lowplots.shp"), encoding="utf-8")
    ...
    if pockets:
        gpd.GeoDataFrame({
            ...
            crs=CRS).to_file(os.path.join(out_dir, "W4_sls_candidates.shp"), encoding="utf-8")
    ...
    gpd.GeoDataFrame({"LABEL": ["OF-1"], "GND": [round(of_rep["z"], 3)],
        ...
        os.path.join(out_dir, "W4_outfall.shp"), encoding="utf-8")

    gpd.GeoDataFrame({"NAME": ["test boundary (repaired)"]}, geometry=[boundary],
                     crs=CRS).to_file(os.path.join(out_dir, "W4_boundary.shp"),
                                      encoding="utf-8")
