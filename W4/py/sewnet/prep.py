...
def clip_roads(roads_path, boundary, sliver_m=0.5):
    """Road LineStrings clipped to the boundary, exploded single-part, slivers dropped."""
...
def prepare(roads_path, boundary_path, terrain_path, sliver_m=0.5, dual_merge_m=35.0):
    """Full S1: returns (segs, boundary, sampler, stats)."""
    boundary = load_boundary(boundary_path)
    clipped = clip_roads(roads_path, boundary, sliver_m)
    segs = node_roads(clipped)
    dual_idx = detect_duals(segs)
    segs2 = collapse_duals(segs, dual_idx, dual_merge_m)
    segs2 = node_roads(gpd.GeoSeries(segs2))   # re-node after re-anchoring
    sampler = TerrainSampler(terrain_path, boundary.bounds)
    stats = {"boundary_ha": boundary.area / 1e4, "segs_raw": len(segs),
             "dual_flagged": len(dual_idx), "segs_final": len(segs2),
             "len_km": sum(s.length for s in segs2) / 1000.0}
    return segs2, boundary, sampler, stats
