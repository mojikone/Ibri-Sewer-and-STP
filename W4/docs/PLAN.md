...
1. **Test outfall — DECIDED (user, 2026-08-18):** pipeline auto-selects the lowest road node on the boundary; user expects ≈ E449915.23 N2567618.10 (EPSG:32640, west edge). S1 cross-checks and REPORTS the comparison but does NOT stop (user 2026-08-18: no confirmation gate). The trunk-connection input stays in the config for study-wide runs.
1c. **Tertiary connectability is mandatory, not cosmetic (user, 2026-08-18):** roads are locally elevated (flood protection, underpasses) so houses can sit BELOW road level. Every plot gets an elevation check: plot ground (0.5 m terrain) minus 0.6 m outlet depth must reach its manhole's invert with ≥2% fall over the connection path. Failing plots = LOW_PLOT flag; manholes with failing clusters get a deepening pass (extra node depth, capped by the 12 m rule) and the invert solver re-runs. Remaining failures are reported as local-solution candidates — never silently dropped.
1d. **T01 tutorial → Rev 3 (user, 2026-08-18):** add a Colebrook-White chapter (equation, ks = 1.5 mm, viscosity 1.141e-6 m²/s, partial-full hydraulics, how Table 11's minimum gradients derive from CW @ 0.75 m/s, worked example) — comprehensive but concise, human tone. Regenerate docx/pdf + md digest.
1b. **Methodology report: the executive summary is the priority section** (user, 2026-08-18) — the most concise yet complete version of the whole story, human tone, smooth to read. Written last, placed first.
