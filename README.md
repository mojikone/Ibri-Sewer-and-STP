...
| Date | Update |
|---|---|
| 2026-08-16 | **Towns polygon layer from client KMZ**: `Final_Boundary_IBRI.kmz` parsed → `Hydraulic/SHP/Towns/Towns.shp` (25 towns, EPSG:32640, UTF-8 DBF) with `NAME_EN`/`NAME_AR`/`CODE` (NCSI national code, from points) + `TOWN` census code and **NCSI population projections Pop_2023…Pop_2050** carried from the polygons — closes the pending occupancy/population gap source; 2025 total 119,244 → 2050 212,643 (IBRI 68,713 → 122,535). AL MUQAYDIH point has no polygon in the KMZ (data request); boundary polygon exported separately (521.1 km² computed vs 507.49 in attribute) |
