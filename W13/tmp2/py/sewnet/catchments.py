...
def build(net, per_chamber, units, trunk_keys, boundary, stubs=None, crit=DEFAULT):
    ...
    # Label the tier the way the as-built designer did (5A-2-SM.2-MH391 = package,
    # sub main, manhole). A sewer carrying 100 properties or more is doing the job of a
    # sub main; the rest are laterals. Learned from NAMA's network on 2026-08-21, where
    # 91% of laterals feed another lateral and only the sub mains touch the trunk.
    SUB_MAIN_PROPS = 100.0
    for r in net.reaches:
        r.tier = ("trunk main" if r.on_trunk else
                  ("sub main" if r.n_props >= SUB_MAIN_PROPS else "lateral"))
