...
for f in sl.getFeatures():
    ...
    note = f"{int(f['POP_TODAY']):,} today -> {int(f['POP_ULT']):,} at 2072 | full {sat_y if sat_y else 'never'} | {f['Q_ULT']:.0f} m3/d"
...
title = 'Settlements merged: people today and at ultimate (2072), the year each fills, sewage at ultimate. Plots: built by use, empty green = takes people'
