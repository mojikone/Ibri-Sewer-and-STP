...
class ChamberPlacer:
    ...
    # ---------------- bends ----------------
    def _in_a_plot(self, x, y):
        """Is this point inside somebody's plot?

        The corner rule only ever guarded BEND chambers, so junctions, heads and spacing
        chambers were free to land in a garden — 49 of them did, one 4.8 m inside a house
        (found by the user 2026-08-21). Every chamber is checked now."""
        if self.plots is None:
            return False
        from shapely.geometry import Point
        p = Point(x, y)
        for i in self.plots.query(p):
            if self.plot_geoms[i].contains(p):
                return True
        return False

    def nudge_out_of_plots(self, net):
        """Slide any chamber that landed inside a plot back onto its own pipe.

        A chamber is moved along the pipe it belongs to, a metre at a time, until it is out
        of the plot — the pipe alignment is not changed, only where the chamber sits on it.
        Anything that cannot be freed is reported rather than left silently in a garden."""
        if self.plots is None:
            return {"checked": 0, "in_a_plot": 0, "moved": 0, "stuck": 0}
        outg, inc = net.outgoing(), net.incoming()
        found = moved = stuck = 0
        for k, ch in list(net.chambers.items()):
            if ch.kind == "outfall" or ch.on_trunk:
                continue
            if not self._in_a_plot(ch.x, ch.y):
                continue
            found += 1
            line = None
            for r in (outg.get(k) or []) + (inc.get(k) or []):
                if r.length > 4.0:
                    line = r
                    break
            if line is None:
            ...
            for step in range(1, 26):
                d = step * 1.0
                pt = (line.geom.interpolate(d) if at_start
                      else line.geom.interpolate(max(0.0, line.length - d)))
                if not self._in_a_plot(pt.x, pt.y):
                    ch.x, ch.y = pt.x, pt.y
                    ch.z = float(self.sampler.z(pt.x, pt.y))
                    ok = True
                    break
            moved += int(ok)
            stuck += int(not ok)
        return {"in_a_plot": found, "moved": moved, "stuck": stuck}
