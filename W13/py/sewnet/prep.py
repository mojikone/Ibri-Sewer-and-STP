...
def prepare(roads_path, boundary_path, terrain_path, sliver_m=0.5, dual_merge_m=35.0,
            hazard_path=None, terrain_pad=200.0):
    ...
    # the main pipe runs on past the boundary to reach the outfall, so the terrain window
    # has to cover that lead too (user 2026-08-20: outfall sits 792 m outside)
    sampler = TerrainSampler(terrain_path, boundary.bounds, pad=terrain_pad)
    hazard = (HazardSampler(hazard_path, boundary.bounds, pad=terrain_pad)
              if hazard_path else None)
