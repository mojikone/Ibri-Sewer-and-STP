...
class SewerDesignPipeline:
    ...
    def run(self):
        ...
        try:
            ...
            _pl = _gpd.read_file(cfg.plots)
            if _pl.crs is None or _pl.crs.to_epsg() != 32640:
                _pl = _pl.set_crs(32640, allow_override=True)
            self.all_plots = [g for g in _pl.geometry
                              if g is not None and not g.is_empty
                              and g.intersects(boundary)]
            self.log(f"   {len(self.all_plots)} plot outlines held for chamber clearance")
        except Exception as _e:
            self.all_plots = None
            self.log(f"   (could not load all plots for clearance: {_e})")
        self.log(f"   {lstats}")
        ...
        # ANY pipe that crosses a dual carriageway needs trenchless work, not only the
        # crossings this code created. Marking just its own left 7 of 8 real crossings
        # paying nothing (found by the user 2026-08-21).
        if cfg.treat_roads:
            from shapely.ops import unary_union as _uu
            _dual = [g for g, why in rt.removed if why.startswith("dual")]
            if _dual:
                _dg = _uu(_dual)
                _n = 0
                for _u, _v, _d in Gu.edges(data=True):
                    if _d.get("crossing"):
                        continue
                    if _d["geom"].crosses(_dg):
                        _d["crossing"] = True
                        _d["free_crossing"] = False
                        _n += 1
                self.reports["road_treatment"]["unplanned_crossings_charged"] = _n
        ...
        for attempt in range(cfg.reroute_passes + 1):
            ...
            # EVERY plot, not just the ones carrying load. A chamber must stay out of an
            # empty plot as much as a built one, and checking only loaded units left 29
            # chambers sitting in gardens (found 2026-08-21).
            plot_shapes = list(getattr(self, "all_plots", None) or
                               [u.geom for u in units if getattr(u, "geom", None) is not None])
            ...
            nud = placer.nudge_out_of_plots(net)
            if nud.get("in_a_plot"):
                self.reports["plot_clearance"] = nud
                self.log(f"   {nud['in_a_plot']} chambers had landed inside a plot: "
                         f"{nud['moved']} slid back onto the pipe, {nud['stuck']} stuck")
            Labeller.run(net)
