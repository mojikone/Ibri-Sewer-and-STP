...
def layers():
    ...
    cats.append(QgsRendererCategory('Unmetered', QgsFillSymbol.createSimple(
        {'style': 'no', 'outline_color': '#9a9a9a', 'outline_width': '0.07', 'outline_width_unit': 'MM'}), 'Empty plot, outline only'))
    pu.setRenderer(QgsCategorizedSymbolRenderer('DERIVED', cats))

    # the settlements shaded by their saturation flow, each labelled with it
    qs = QgsVectorLayer(os.path.join(SHP14, "Settlements_merged.shp"), "Average sewage flow at saturation, m³/d", "ogr")
    ...
    _label(qs, """title("SETTLE") || '\\n' || format_number("Q_ULT", 0) || ' m³/d'""", 7.2, NAVY, bold=True, buffer=1.0)
    ...
    fm.setRenderer(QgsCategorizedSymbolRenderer('DWELLING', [
        QgsRendererCategory(1, QgsMarkerSymbol.createSimple(
            {'name': 'circle', 'color': '#C0504D', 'outline_color': '#ffffff', 'outline_width': '0.25', 'size': '2.4'}), 'Dwelling meter'),
    ...
    fmb.setRenderer(QgsCategorizedSymbolRenderer('DWELLING', [
        QgsRendererCategory(1, QgsMarkerSymbol.createSimple(
            {'name': 'circle', 'color': '#C0504D', 'outline_color': '#ffffff', 'outline_width': '0.4', 'size': '3.6'}), 'Dwelling meter'),
        QgsRendererCategory(0, QgsMarkerSymbol.createSimple(
...
FIGURES = {
    ...
    "B03_saturation": ("Average sewage flow of each settlement at saturation",
                       ["Project Boundary updated", "Average sewage flow at saturation, m³/d"], "B03_saturation"),
    ...
    "B05_free_meters": ("The meters more than 15 metres from any plot, and the zoom panels",
                        ["Project Boundary updated", "Meter more than 15 m from any plot", "Zoom panel (appendix)",
