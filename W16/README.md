...
| `report/` | **The Concept Design Report, Revision 3** (`R3/`, 107 pages, 2026-09-17, numbered by chapter 1 to 8; R0 to R2 stay in `W14/report/`): `build.py --pages`, the charts added for R3 in `charts_r3.py`, the study-area section in `rpt_area.py` with `facts_area.py`, `charts_area.py` and `qgis_maps_area.py` (data in `analysis/study_area/`), the deliverables list in `deliverables.py`, sections in `rpt_front`, `rpt_ab`, `rpt_c`, `rpt_d`, `rpt_ef`, `rpt_gh`, `rpt_app`, the decisions through `basis_items.py` from `report_basis/decisions.py`; plus the shared furniture every report uses: `doc.py`, `notes.py`, `omml.py`, `flow.py`, `charts.py`, `qgis_maps.py`, `facts_w14.py`, `to_pdf.py`, and the Word template in `template/` |
...
| `meeting/` | **The meeting folder's builders** (2026-09-15): `make_meeting_folder.py` copies every latest layer, the reports, the tables and the delivered zip into `W16/Meeting 2026-09-16/` (not in git: rasters and the client's data), clips the five flood-hazard rasters, resamples the terrain to 2 m with a hillshade and exports stp2's styles from the running QGIS; `make_meeting_project.py` (standalone QGIS python) assembles `Ibri_meeting_2026-09-16.qgz` with relative paths and the report-map copies. `MEETING_README.md` is the folder's own README |
