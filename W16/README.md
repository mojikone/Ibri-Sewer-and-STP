...
| `report/` | **The Concept Design Report, Revision 3** (`R3/`, 95 pages, 2026-09-17, numbered by chapter 1 to 8; R0 to R2 stay in `W14/report/`): `build.py --pages`, the charts added for R3 in `charts_r3.py`, the deliverables list in `deliverables.py`, sections in `rpt_front`, `rpt_ab`, `rpt_c`, `rpt_d`, `rpt_ef`, `rpt_gh`, `rpt_app`, the decisions through `basis_items.py` from `report_basis/decisions.py`; plus the shared furniture every report uses: `doc.py`, `notes.py`, `omml.py`, `flow.py`, `charts.py`, `qgis_maps.py`, `facts_w14.py`, `to_pdf.py`, and the Word template in `template/` |
