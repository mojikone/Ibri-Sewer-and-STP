...
Outputs: `W14/shp/ELE_meters_on_plots.shp`, `W14/shp/PLOTS_load.shp` (56 fields: cadastre, meters by tariff and category,
use and rule, shape and satellite tests, the settlement's adopted rates `OR_S` `PPP_S` `HOMESH_S` `U_NDOM` `U_GOV`, people,
water and sewage by stream, `QADF`, people and flow 2030 / 2055 / saturation; the test and audit fields are in
`W14/analysis/plot_class_audit.csv`), `W14/shp/Settlements_merged.shp` (69 fields: derived and adopted rates, meter counts,
shares and unit rates, five-year steps, saturation with and without overflow, people sent and received),
`W14/analysis/W14_growth_by_settlement.xlsx`; QGIS group *Claude W14 load*. Field dictionaries: report Appendix A4.
...
| 3 | Non-domestic and governmental water is a settlement pool from its people (× 0.22 × 164 and × 0.14 × 164), landed on the settlement's shop and government meters in equal shares per meter — a **unit rate per meter** (`U_NDOM`, `U_GOV`, L/d; Ibri 510 and 3,342); **the two estates' meters stay out of the pool** (engineer 2026-09-10); a settlement with no such meter keeps the share on its dwellings. Per existing plot, L/d: **Q = 0.85 × 164 × OR × N_dom + 0.54 × (U_nd × N_nd + U_gov × N_gov + 93 × workers)** | locked basis, PROJECT-STATE §2 1b; report 15.4 |
| 4 | Special = the two industrial estates: **4,500 workers Al Tayyeb, 1,800 Tanam**, spread over their industrial plots by area, **93 L/d each** (dry industry); an estate plot also carries any dwelling meters on it at the domestic rate (60 plots, 129 meters), and no share of the shop pool | Tab 12, G1-p61; engineer's assumption |
...
| 6 | A meter outside every plot snaps to the nearest plot within **15 m**; else it stays free, belongs to the nearest settlement and **carries no load in the plot table** (126 meters: 30 dwellings, 47 government, 44 shop, 5 farm; about 130 people, 0.1 % of the flow — snapping them regardless of distance is the engineer's call) | engineer |
...
| Properties | 22,559 → **119,893 people** with the per-settlement occupancy (workbook 2024 for the 25: 116,452; the floor at 4.0 and the under-1,000 rule add 3,441 net) |
| Water | 27,327 m³/d |
| **Sewage (Qadf)** | **20,852 m³/d** |
...
| Capacity | **229,136 people** (v6 before the under-1,000 rule: 247,671; at 5.32 everywhere: 274,565; all empty plots ≤ 2,000 m² at the ratio, v4: 369,172; v1 loose class: 407,411) |

## 3. Growth

| Year | New people housed | Total people | Nowhere to go | Qadf m³/d |
|---|---|---|---|---|
| 2030 | 19,643 | 139,535 | 0 | 24,216 |
| 2055 | 125,020 | 244,914 | 0 | 42,266 |
| **2070 ultimate** | 229,136 | **349,029** | 344 | **60,099** |
| 2100 (workbook) | 229,136 | 349,029 | 362,659 | 60,099 |
