...
| 71 | **Page numbers**: none on the cover; lower-case roman from **i** at the Contents over the front matter (contents, lists, abbreviations, executive summary, i to xi); arabic from **1** at Chapter 1 to the end (1 to 85), unbroken through the landscape pages and Appendix A. The cover is its own section; the numbering of every section is written once before saving, because a section added for a landscape page copies the start value of the one before it | `doc.front_matter`, `doc.body_start`, `doc.number_pages`, `build.py` |
| 72 | **Chapter headings more visible**: 24 pt, 100 pt above and 100 pt below. The space above is an empty line of exactly 100 pt carrying the page break, because Word drops a heading's space-before on a page opened by a page break and keeps it at a section start, which put Chapter 1 at one height and the rest at another | `doc.chapter` |
| 73 | **Section 1.2 is now "The study area"** and describes it (engineer, 2026-09-17): 1.2.1 to 1.2.3 as before, then **1.2.4 topography and drainage** (map A01: hillshade, ground-level tint, wadis of order 3 and above, the STP; chart S01: ground level of every settlement in order of distance from the STP, the two below it in red), **1.2.5 climate** (BWh; chart S02 monthly temperature and rainfall; chart S03 four wind roses; a table of the ten settlements within 8 km of the STP with the wind that carries air from the STP to each, share of all hours and of night hours), **1.2.6 road network** (map A02 by the class code as supplied, chart S04), **1.2.7 flood hazard** (the six Australian classes with their limits; maps A03 to A06 for 10, 25, 50 and 100 years, the engineer's colours at 50 %, the ground outside the boundary veiled). Cross-references added in the executive summary, 7.2 and 7.3 | `rpt_area.py`, `facts_area.py`, `charts_area.py`, `qgis_maps_area.py`, layouts `RPT A01…A06` saved in stp2.qgz |
| 74 | **Figure 2 carries a locator inset**: Oman by governorate, Adh Dhahirah shaded, the Wilayat of Ibri in blue, the study area in red. Outlines from OpenStreetMap, downloaded 2026-09-17 (11 governorates; geoBoundaries still holds the 7 regions of 2010). The map now sits ahead of 1.2.1 | `qgis_maps._locator_inset`, `analysis/study_area/fetch_admin_osm.py` |
| 75 | **Public climate record**: NASA POWER (MERRA-2) at the STP, because no station record has been supplied. Temperature and rainfall 2001 to 2020, wind 2015 to 2024. **The rainfall after 2020 is rejected**: POWER carries 790 mm for August 2024 at this point. The report says the record describes the regime and does not replace the station record the odour modelling needs | `analysis/study_area/` |
| 76 | **The main pipe is not shown on these maps: it is not client data** (engineer, 2026-09-17) | `qgis_maps_area.py` |
| 77 | Flood hazard: source **Oman Flood Mapping project, MAFWR**; classes **Australian, H1 low to H6 high** (AIDR Guideline 7-3, 2017), limits checked against the guideline on 2026-09-17. Maps only, no exposure statistics (engineer) | `facts_area.HAZARD_CLASSES` |
| 78 | Decision 1's box moved after its map in 4.1.2: ahead of it the box was stranded alone on a page | `rpt_d.py` |

R3 stands at **107 pages**: the cover, i to xi, 1 to 95.
