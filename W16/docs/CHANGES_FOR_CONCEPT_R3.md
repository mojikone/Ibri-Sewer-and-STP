...
| 71 | **Page numbers**: none on the cover; lower-case roman from **i** at the Contents over the front matter (contents, lists, abbreviations, executive summary, i to xi); arabic from **1** at Chapter 1 to the end (1 to 83), unbroken through the landscape pages and Appendix A. The cover is its own section; the numbering of every section is written once before saving, because a section added for a landscape page copies the start value of the one before it | `doc.front_matter`, `doc.body_start`, `doc.number_pages`, `build.py` |

R3 stands at **95 pages**: the cover, i to xi, 1 to 83.
