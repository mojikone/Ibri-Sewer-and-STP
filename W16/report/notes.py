...
_FIRST_ID = 1        # -1 and 0 are the separator footnotes Word reserves (as its settings.xml declares)
...
def _footnotes_xml():
    ...
    for i, kind in ((-1, "separator"), (0, "continuationSeparator")):
        parts.append(
    ...
    with zipfile.ZipFile(path) as zin, \
        ...
        for item in zin.infolist():
            if item.filename == "word/footnotes.xml":
                continue          # the template carries one: ours replaces it
