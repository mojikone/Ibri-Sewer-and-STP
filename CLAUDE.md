...
Concept→detailed design + supervision of wastewater network, treated-effluent (TE) network and STP capacity for Ibri Wilayat, Oman (Client: Nama Water Services, Tender T/2719110/2025). Design horizon completion+25 yr or saturation; model years start/2030/2055/ultimate; SewerGEMS/WaterGEMS deliverables; ≥3 options each for sewer network, TE network and each STP. Existing STP at E444387 N2563352 (EPSG:32640, ground ≈327.5 m). Saturation Qadf **60,099 m³/d in 2070 at the concept stage** (W14, per plot; the old ≈ 49,700 is superseded — design flows: `W14/docs/DESIGN_FLOWS_FOR_NETWORK.md`) (>20,000 threshold → STP phasing is the pivotal decision).

## Working rules (user-mandated)
1. **Iterations — see global rule 12.** Here the folders are `W1/`…`W9/` and `W13/`; the current design is named in `_BRAIN/00_CURRENT.md`.
   ...
   **AND IT COPIES THE BEST ONE, NOT THE LAST ONE. W13 IS A COPY OF W8.**
   W10 scaled from 5.51 km² to 531 km² — ninety-six times — in a single step, and was never
   required to reproduce W8's answer on W8's own ground first. W11a, W11b and W12 each
   inherited that. Measured 2026-09-06 inside W8's own boundary, W12 produced **3,139 chambers
   against W8's 1,415, and one pumping station where W8 and the network NAMA actually built
   both need none.** W10–W12 were reverted out of this repo on 2026-09-07 by the engineer's
   decision. Their history is in tag and branch `w12-archive-2026-09-06`; the knowledge they
   produced is quarantined (rule 14) and their outputs are in `Backup/1 Claude.rar`.

   **THE GATE, and it is the whole lesson: no change ships until the design still gives
   71.6 km, ~1,415 chambers and ZERO pumping stations on W8's 5.51 km² test boundary.**
   `W13/py/run_test_boundary.py` does it in **26 seconds**. Run it after every change. A design
   that cannot still do 5.51 km² has no business being trusted on 531.
...
9. SLS: one station per contiguous non-gravity pocket (12 m max cover rule, GUD-203 p33). *The cascade-within-1.5 km and the under-50-plots deferral were dropped by the engineer on 2026-09-12: every pocket is a pump candidate in stage B, cascading is a stage C decision.*
...
14. **THE QUARANTINE — `Hydraulic/Backup/2 Quarantine_W10_W12/`.** The six weeks that
    produced four worse designs also produced this project's best measurement: the as-built
    calibration, the depth-versus-pumping economics, the hierarchy rules read from NAMA's own
    manhole IDs, two solver studies and the W8/W10 post-mortem. The engines are not worth
    keeping; some of the knowledge may be. **NOTHING IN THAT FOLDER IS TO BE USED UNLESS THE
    USER ASKS FOR IT BY NAME.** Judge any file in it by one question — *does this help W13 beat
    W8 on W8's own 5.51 km²?* — because the failure being corrected was adding machinery faster
    than engineering, and re-importing it wholesale would repeat that exactly.
...
| `TUTORIALS/` | **T04 — the whole method in one tutorial**: meter → plot → flow → self-cleansing sewer → plant → appraisal, every step with its rule, guideline page, worked Ibri number and the script and field that does it, Word + PDF from one source. **T01, T02 and T03_R01 are frozen as the record** |
...
| **`W14/`** | **Population, saturation and load per plot (class v7), the Concept Design Report R2 (`report/R2/`, live; R0 and R1 frozen alongside) and `deliverables/`.** Read `W14/docs/CONCEPT_NOTE_SATURATION_AND_LOAD.md` first; the report reads its numbers from `W14/report/facts_w14.py`. **Designing the network or the plant: read `W14/docs/DESIGN_FLOWS_FOR_NETWORK.md`** (which field and year for which check; size on `Q_ULT`, self-cleansing on `Q_2030` × 0.61) |
| **`W15/`** | **The live pipe-laying engine, the whole network** (since 2026-09-12): a complete copy of `W13/tmp4`, logic `W15/docs/W15_DESIGN_LOGIC.md`, purpose and settings in `W15/README.md` |
| **`W16/`** | **The reports (since 2026-09-14): the Design Basis Report R0 (`W16/report_basis/`, for approval ahead of the concept design) and the concept report build for R3 (`W16/report/`), on the firm's template with native captions; the loads stay in `W14/shp`. What R3 must carry: `W16/docs/CHANGES_FOR_CONCEPT_R3.md` |
| `W13/` | The engine's record on the test boundary: `tmp2` (the accepted layout), `tmp3` (plot flows, audit), `tmp4` (stage B, pumps), all frozen; plus the first population work that W14 now carries |
...
| `../QGIS/QGIS 2621 ibri sewer stp2.qgz` | Live QGIS project since 2026-09-12 (layers + saved layouts W2 M1–M6); `stp.qgz` is the older copy |
