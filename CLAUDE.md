...
9. SLS: consolidate — one station per contiguous non-gravity pocket (12 m max cover rule, GUD-203 p33), ~~cascade stations within ~1.5 km~~, absorb pockets <50 plots to detail design.
   **REVERSED 2026-09-06 (user): a station NEVER pumps into another station** — it discharges to a gravity manhole or to the works. *"If the pump is close, the number of pumps is not optimised; if they are far, it is not advised to pump sewage for long distances."* Cascading is a symptom of bad siting at every separation. See `_BRAIN/08_DESIGN_PHILOSOPHY.md` §6. The struck clause is kept as the record per rule 12.
