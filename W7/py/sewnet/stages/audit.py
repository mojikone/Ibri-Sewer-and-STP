...
def _inlet_angle(ctx):
    ...
    STATED DEVIATION (user, 2026-08-20): the threshold is the project value INLET_MIN_DEG,
    75 deg, not the guideline 90. Holding 90 meant inserting a bend chamber a few metres
    before roughly 200 junctions, which buys nothing on site and costs a chamber each. What
    is left is FLAGGED for a designer to look at, never fixed by adding chambers."""
    bad = []
    ...
    lim = ctx.crit.INLET_MIN_DEG
    return ("FAIL" if bad else "PASS",
            f"{len(bad)} of {n_in} inlets under {lim:.0f} deg "
            f"(project value; guideline is 90 - see criteria register)", bad)
...
def _pcs_length(ctx):
    """PARKED by the user on 2026-08-20: the drawn house connections were not good enough,
    so this iteration does not draw them and the length rule has nothing to measure. It is
    reported as NOT CHECKABLE, never as a pass — a skipped row must not read as compliance.
    The check returns as soon as the survey gives real frontages."""
    if not ctx.conn:
        return ("NOT_CHECKABLE",
                "house connections are not drawn in this iteration (user 2026-08-20); "
                "returns with the survey frontages", [])
    C = ctx.crit
...
REGISTRY = [
    ...
    Check("C4", "Chambers", "Inlet angle", "G203-p30 (deviation: 75 deg)",
