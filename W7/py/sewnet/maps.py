...
def network_map(path, net, pockets, of_rep, boundary, summary):
    ...
    box = (f"W7 TEST-BOUNDARY SEWER DESIGN\n"
    ...
    _frame(ax, boundary, "W7 — Test-Boundary Gravity Sewer Network (by pipe size)", box)
...
def depth_map(path, net, boundary, summary):
    ...
    _frame(ax, boundary, "W7 — How deep the pipes sit below ground", box)
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
...
def lowplot_map(path, net, conn, still_low, boundary, summary):
    ...
    _frame(ax, boundary, "W7 — Can every house drain into the sewer?", box)
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
...
def catchment_map(path, net, cats, boundary, summary):
    ...
    _frame(ax, boundary, "W7 — what each subnetwork collects", box)
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
