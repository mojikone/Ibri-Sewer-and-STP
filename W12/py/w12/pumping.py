"""w12.pumping - PUMPING STATIONS AND FORCE MAINS, DESIGNED.
...
THREE CITATION DEFECTS FOUND WHILE WRITING THIS, ALL AGAINST THE SOURCE PDF
    (1) `_BRAIN/08_DESIGN_PHILOSOPHY.md` sec 3, flood table, says a pumping station's floor
        ...
        p63 Table 27 row (i), which is STP SITE SELECTION, not a pumping station. W12's
...
WHAT THIS FILE IS NOT
    ...
    reads `w12.criteria` for every value that file already holds, and declares here - with
...
PUMPING_VERSION = "W12-pumping-1.0"
...
class PumpCriteria:
    ...
    Frozen, like `Criteria`, so a sensitivity run is a new object and never an edit:
        ...
        from w12.pumping import PUMP, replace
...
def optimise_force_main(q_duty_ls: float, fl: Flows, length_m: float, static_lift_m: float,
    ...
    So the smallest legal size is NOT the answer, and W12 said it was until the as-built
...
def size_force_main(q_duty_ls: float, fl: Flows, length_m: float, static_lift_m: float, *,
    ...
    that is what G203-p50 demands in terms. W12 previously took the SMALLEST legal size on
...
def site_station(x: float, y: float, ground_m: float, grids, *,
    ...
    `grids` is a `w12.hazard.HazardGrids`.
...
def banner(pc: PumpCriteria = PUMP, crit: Criteria = DEFAULT) -> str:
    return "\n".join([
        f"W12 pumping {PUMPING_VERSION} - flags that travel with every number below:",
