# -*- coding: utf-8 -*-
"""w12.present — the presentation library. One style spec, two outputs: KMZ and QGIS.
...
-------------------
  ...
  * It depends on NO W12 stage. Hand it a GeoDataFrame or a path; it neither knows nor
    cares which stage produced it. That is deliberate: the library was written and proven
    against W11a's layers before W12 had any of its own.
...
PRESENT_VERSION = "w12-present-1.0"
...
# Names in `w12.criteria` that hold the same quantity as a row of G above. This module does
...
def verify_against_criteria() -> List[str]:
    """Compare every number this module quotes with `w12.criteria`, if that module is
    importable. Returns a list of disagreements — empty is the passing result.

    Checked 2026-09-03 against `w12/criteria.py` (W12-criteria-1.0): all eleven overlapping
    values agreed exactly, including the tau sensitivity factor, which both files put at
    2 ** 1.23 = 2.3457."""
    try:
        from w12.criteria import DEFAULT as C          # type: ignore
    except Exception as e:
        return [f"w12.criteria not importable ({type(e).__name__}); nothing cross-checked"]
    bad = []
...
def derive_ground_slope(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This is the W12 headline metric. W11a laid its tree on road connectivity and used the
    ...
    this column shows in one glance whether W12 actually fixed it.
...
register(View(
    ...
    question="THE W12 QUESTION. How much of this network carries flow UPHILL?",
...
def _ground_fall_stats(df: pd.DataFrame, lengths) -> Dict[str, Any]:
    """The headline W12 number: how much length runs uphill, and by how much."""
...
_QGIS_LOADER = r'''
# --- generated by w12.present. Run inside QGIS (Python console, or the qgis MCP). --------
import json, os
...
def apply_spec(lyr, spec):
    ...
    lyr.setCustomProperty("w12_question", spec.get("question", ""))
    lyr.setCustomProperty("w12_flags", " | ".join(spec.get("assumptions", [])))
    lyr.triggerRepaint()
...
for lname in PLAN.get("layouts", []):
    ...
    if disp:
        ...
        report["layouts"].append(build_layout("W12 " + lname.upper(), disp, ban))
...
def _cli(argv=None):
    ...
    ap = argparse.ArgumentParser(
        description="w12.present — build the KMZ set and the QGIS loader from a layer set.")
    ...
    ap.add_argument("--prefix", default="W12")
    ap.add_argument("--group", default="Claude W12")
    ...
    ap.add_argument("--verify", action="store_true",
                    help="cross-check every quoted number against w12.criteria and exit")
    ...
    if a.verify:
        ...
        print("\n".join(bad) if bad else
              f"present.py agrees with w12.criteria on all "
