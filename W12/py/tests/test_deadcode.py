...
def _sources():
    return sorted(PY_DIR.glob("*.py")) + sorted((PY_DIR / "w12").glob("*.py"))
...
@pytest.mark.parametrize("mod,bound_s", [
    ("w12.criteria", 15.0),      # measured 0.3 s
    ("w12.hydra", 15.0),         # measured 0.1 s
    ("w12.contract", 20.0),      # measured 1.1 s
    ("w12.pumping", 20.0),       # measured 0.1 s, 34 self-checks
    ("w12.hazard", 30.0),        # measured 0.4 s, reads the shipped grids if present
    ("w12.asbuilt", 60.0),       # measured 11.5 s, reads NAMA's 188.6 km as-built
])
def test_module_self_test_passes_inside_its_budget(mod, bound_s):
    """`python -m w12.<module>` is each module's own self-test. Running them from the
