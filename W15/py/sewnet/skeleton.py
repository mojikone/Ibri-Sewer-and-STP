            ...
            for c in cand:
                ...
                if cut is None:
                    ...
                    if conn_nodes is None:
                        continue
                    cut = len(seq_nodes) - 1
                # temp 2 (2026-09-08): a sub main is laid on its own outlet's ground only.
                # A street piece keeps the boundary node of the next catchment at its
                # upstream end (the run between them goes with the side the water leaves
                # toward), and that node's outflow must stay with its own outlet: with it
                # on the stem, a pocket's sub main hoisted 40 km of the works' catchment
                # into the pocket. The piece is the stretch of the outlet's own nodes
                # ending at the attach node; the boundary run is left to be a branch
                start = cut
                while start > 0 and src.get(seq_nodes[start - 1]) == outlet:
                    start -= 1
                if start == cut or src.get(seq_nodes[cut]) != outlet:
                    continue
                part_nodes = seq_nodes[start:cut + 1] + conn_nodes
                part_runs = seq_runs[start:cut] + conn_runs
                L = sum(runs[i]["len"] for i in seq_runs[start:cut])
                if L < chain_min_m and not (c["len"] >= chain_min_m and L >= 0.2 * chain_min_m):
                    # temp 2 (2026-09-08): the minimum length is the street's, not the piece
                    # inside the sub-network; an outlet part-way along a long street would
                    # otherwise attach nothing and the whole settlement lose its sub mains
                    continue
