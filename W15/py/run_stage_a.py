...
def main():
    ...
    if getattr(cfg, "AREA_MODE", "") == "DXF":
        # W15: the area is every street the draftsman drew; the settlements are the dense
        # parts of it (the 2006 network is a reference only, engineer 2026-09-12)
        all_lines = G.read_dxf_lines(cfg.ROADS_DXF, cfg.ROAD_LAYERS)
        envelope = unary_union([g_.buffer(cfg.AREA_BUFFER_M, 4) for g_, _ in all_lines]).simplify(2.0)
        parts_ = list(getattr(envelope, "geoms", [envelope]))
        from shapely.geometry import MultiPolygon
        built_env = MultiPolygon([q for q in parts_ if q.area >= cfg.SETTLEMENT_MIN_HA * 1e4])
        rep["area_source"] = "every street of the DXF"
        rep["settlements"] = len(built_env.geoms)
        log(f"   area from the DXF: {envelope.area / 1e6:.1f} km2 in {len(parts_)} pieces, "
            f"{len(built_env.geoms)} settlements of {cfg.SETTLEMENT_MIN_HA:.0f} ha and more")
    elif getattr(cfg, "AREA_SHP", None):
    ...
    if getattr(cfg, "GUIDE_SHP", None) and os.path.exists(cfg.GUIDE_SHP):
        # W15: the engineer's sub main guide (2026-09-12): its lines are streets of the
        # network, forced sub mains in the direction drawn, a join where they meet the main
        # pipe; an end within GUIDE_SNAP_M of the main pipe is tied to it with a connector
        _mp_all = unary_union([g_ for g_ in gpd.read_file(cfg.MAIN_PIPE).geometry if g_ is not None])
        for g_ in gpd.read_file(cfg.GUIDE_SHP).geometry:
            if g_ is None or g_.is_empty:
                continue
            coords_ = [(x, y) for x, y, *_ in g_.coords]
            end_ = Point(coords_[-1])
            d_ = _mp_all.distance(end_)
            if 0.5 < d_ <= cfg.GUIDE_SNAP_M:
                foot_ = _mp_all.interpolate(_mp_all.project(end_))
                coords_.append((foot_.x, foot_.y))
            guide_lines.append(LineString(coords_))
        lines += [(g_, "guide") for g_ in guide_lines]
        rep["guide_km"] = round(sum(g_.length for g_ in guide_lines) / 1000, 2)
        log(f"   the engineer's sub main guide: {len(guide_lines)} lines, {rep['guide_km']} km, tied to the main pipe")
    ...
    guide_runs, guide_stem, guide_feet = set(), {}, set()
    if guide_lines:
        for i_, r_ in enumerate(runs):
            mid_ = r_["geom"].interpolate(0.5, normalized=True)
            for gl_ in guide_lines:
                if gl_.distance(mid_) < 1.0 and gl_.distance(Point(r_["up"])) < 1.5 and gl_.distance(Point(r_["dn"])) < 1.5:
                    a_, b_ = gl_.project(Point(r_["up"])), gl_.project(Point(r_["dn"]))
                    up_, dn_ = (r_["up"], r_["dn"]) if a_ < b_ else (r_["dn"], r_["up"])
                    guide_runs.add(i_)
                    guide_stem[up_] = dn_
                    if gl_.project(Point(dn_)) >= gl_.length - 1.5:
                        guide_feet.add(dn_)
                    break
        rep["guide_runs"] = len(guide_runs)
        log(f"   guide: {len(guide_runs)} runs on it, {len(guide_feet)} feet")
    rep["runs"] = len(runs)
    ...
    # temp 2: a pocket is taken OUT of the network, never made an outlet. A basin that
    # needs more than BASIN_MAX_M of fill to reach any target, or one cut after the lay for
    # passing 12 m, loses its own streets (the raw catchment of its sink) from the flood,
    # and the rest is flooded again with the real targets only. So a pocket holds only
    # what cannot reach the outfall, and never pulls a neighbour in that could
    cut_nodes = set()
    pocket_log = []
    pocket_fill = {}

    def lake(sink, f2):
        """The pocket is the lake: the junctions of the sink's own basin that lie below the
        level the water would have to reach to leave it. Streets above that level drain
        elsewhere and are not the pocket's."""
        rim = f2.get(sink, znode[sink])
        return {n for n, t in raw_term.items() if t == sink and znode[n] < rim - 0.05} | {sink}

    def assign_with_pockets(targets_, log_=True):
        for _ in range(60):
            free = [r_ for r_ in runs if r_["up"] not in cut_nodes and r_["dn"] not in cut_nodes]
            live_ = {t: ty for t, ty in targets_.items() if ty != "SINK"}   # a pocket floods nothing
            s2, f2, p2, q2, sp2, it2 = O.resolve_outlets(free, znode, live_, 1.0e9, raw_term,
                                                         **assign)
            deep = [(f2[s_] - znode[s_], s_) for s_ in raw_sinks
                    if s_ in f2 and s_ not in cut_nodes and f2[s_] - znode[s_] > cfg.BASIN_MAX_M]
            if not deep:
                break
            fill_, worst = max(deep)
            members_ = lake(worst, f2)
            cut_nodes.update(members_)
            targets_[worst] = "SINK"
            pocket_log.append((worst, round(fill_, 1), len(members_)))
            pocket_fill[worst] = fill_
            if log_:
                log(f"   pocket: the basin at {tuple(round(v) for v in worst)} needs {fill_:.1f} m of "
                    f"fill to reach any outlet; its lake of {len(members_)} junctions is taken out")
        for n in cut_nodes:
            t = raw_term.get(n, n)
            s2[n] = t
            f2[n] = znode[n]
            pn = parent.get(n)
            p2[n] = pn if (pn in cut_nodes and pn != n) else None
            q2.setdefault(n, 0)
            sp2.setdefault(t, 0.0)
        # second pass: whatever the first pass could not reach without crossing a pocket
        # drains INTO the pocket, to its pump, not to a pocket of its own. The pocket's
        # sinks flood the pocket and the streets cut off behind it, and nothing else
        pocket_sinks = {t for t, ty in targets_.items() if ty == "SINK"}
        island = {n for n, o in s2.items() if o not in live_ and sp2.get(o) is None}
        B = set(cut_nodes) | island
        if pocket_sinks & B:
            runsB = [r_ for r_ in runs if r_["up"] in B and r_["dn"] in B]
            zB = {n: znode[n] for n in B}
            rawB = {n: t for n, t in raw_term.items() if n in B}
            sB, fB, pB, qB, spB, _ = O.resolve_outlets(
                runsB, zB, {t: "SINK" for t in pocket_sinks & B}, 1.0e9, rawB)
            taken = 0
            for n in B:
                if sB.get(n) in pocket_sinks:
                    s2[n], f2[n], p2[n], q2[n] = sB[n], fB[n], pB[n], qB[n]
                    taken += 1
            for t in pocket_sinks & B:
                sp2[t] = pocket_fill.get(t, 0.0)
            if log_ and island - cut_nodes:
                log(f"   pockets: {len(island - cut_nodes)} junctions cut off behind a pocket, "
                    f"{taken - len(cut_nodes & set(sB))} of them drain into it")
        return s2, f2, p2, q2, sp2, it2
    ...
    restored = []
    if guide_feet:
        forced_ = [c for c, i_ in info.items() if i_["type"] == "JOIN" and i_["outlet"] in guide_feet and c not in kept]
        kept += forced_
        dropped_joins = [c for c in dropped_joins if c not in forced_]
        log(f"   guide feet kept as joins: {len(forced_)} restored, "
            f"{sum(1 for c, i_ in info.items() if i_['type'] == 'JOIN' and i_['outlet'] in guide_feet)} in all")
    for _ in range(5):
    ...
    corridor_paths = {}
    if use_corridor:
        entry_nodes = node_keys
        if getattr(cfg, "ROAD_CORRIDOR", False):
            # temp 2: the corridor to the outfall is the roads outside the settlements, so
            # the settlement's way out is a designed pipe along a road, not NAMA's line.
            # Only what the joins on the main pipe cannot take is offered it: the members
            # of the pockets of a flood from the joins alone
            road_gdf = gpd.GeoDataFrame(
                {"US_MHID": ["ROAD-TM-"] * len(lines), "DS_MHID": [""] * len(lines),
                 "PROJECTCOD": ["ROAD"] * len(lines)},
                geometry=[g_ for g_, _ in lines], crs=f"EPSG:{cfg.EPSG}")
            corridor = K.TrunkCorridor(road_gdf, cfg.STP, envelope=built_env)
            src0, _, _, _, spill0, _ = O.resolve_outlets(runs, znode, targets2, cfg.BASIN_MAX_M,
                                                         raw_term, **assign)
            entry_nodes = node_keys           # every junction; the 6.5 km cap sorts them
            log(f"   road corridor: {len(corridor.lines)} road pieces outside the settlements, "
                f"reaches the works: {corridor.ok}; {len(src0) - sum(1 for o in src0.values() if o in targets2)} nodes have no join")
        else:
        ...
        road_mode = getattr(cfg, "ROAD_CORRIDOR", False)
        entries, corridor_paths = K.corridor_entry_targets(
            entry_nodes, znode, ground, corridor,
            None if road_mode else streets,          # the entry stands on a road: the leg
            cfg.LINK_MP_MAX_M, cfg.JOIN_SPACING_M,   # to it crosses nothing by construction
            cfg.CORRIDOR_MIN_GRAD, cfg.CORRIDOR_MAX_M, mp_union=mp_union,
            plots=None if road_mode else gates, stp_level=cfg.STP_INVERT_M)
        entries = {n: t for n, t in entries.items() if n not in targets2}
        if getattr(cfg, "ROAD_CORRIDOR", False):
            # temp 2: a settlement has ONE way out to the outfall, its lowest feasible entry,
            # as NAMA's corner was; entries outside the settlements are dropped, that ground
            # drains to the works by its own streets
            from shapely.prepared import prep as _prep2
            parts = list(getattr(built_env, "geoms", [built_env]))
            keep = {}
            for part in parts:
                pp = _prep2(part.buffer(cfg.LINK_MP_MAX_M))
                inside = [n for n in entries if pp.contains(Point(n))]
                if inside:
                    best_ = min(inside, key=lambda n: znode[n])
                    keep[best_] = entries[best_]
            log(f"   road corridor: {len(entries)} entries found, {len(keep)} kept, one per settlement: "
                + ", ".join(f"{tuple(round(v) for v in n)} at {znode[n]:.1f} m" for n in keep))
            corridor_paths = {n: g_ for n, g_ in corridor_paths.items() if n in keep}
            entries = keep
    ...
    src2, filled2, parent2, seq2, spill2, iters2 = assign_with_pockets(targets2)
    ...
    for _ in range(6 if getattr(cfg, "POCKET_TRUNKS", True) else 0):
    ...
    for rr in range(cfg.REROUTE_ROUNDS + 2):
        ...
        elif getattr(cfg, "OUTLET_CUT_ONLY", False):
            # temp 2: a sub main belongs to one sub-network; a street chain is cut where its
            # two sides drain to different outlets, never at a sag (a huge crest tolerance
            # keeps the sag test from firing)
            chains = K.cut_at_sags_and_outlets(chains, runs, znode, src2, parent2, 1.0e9)
        ...
        stem_parent.update(guide_stem)          # W15: the engineer's guide fixes its own too
        submain = set(submain) | set(trunk_runs) | set(guide_runs)
        ...
        for i in tree_idx:
            runs[i]["tier"] = ("trunk" if i in trunk_runs else
                               "sub main" if i in submain or i in guide_runs else "lateral")
        ...
        if getattr(cfg, "REROUTE_MODE", "trunk") == "cut":
            acc, refused = {}, {}          # temp 2: no reroute; the basin remains a pocket
        else:
            acc, trunk_stem, trunk_runs, levels, ttype, refused = K.trunk_routes(
                runs, znode, levels, ttype, pockets, props_of, cfg.DEPTH_WEIGHT, cfg.MAX_DEPTH_M,
                cfg.TRUNK_COVER_M, cfg.PER_PROPERTY_M3D, stem=trunk_stem, trunk_runs=trunk_runs,
        ...
        for s_ in cut:
            targets2[s_] = "SINK"
            members_ = lake(s_, filled2)
            cut_nodes.update(members_)
            pocket_fill[s_] = filled2.get(s_, znode[s_]) - znode[s_]
            log(f"   rule 10, cut: the basin at {tuple(round(v) for v in s_)} is a pocket for a pump "
                f"({props_of.get(s_, 0)} plots, {len(members_)} junctions taken out)")
    ...
    title = [f"W15 STAGE A - THE WHOLE NETWORK, EVERY STREET OF THE DXF - {date}",
