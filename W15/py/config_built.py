...
OUT = BASE + "/Hydraulic/Claude/W15"   # W15: the main working folder, a complete copy of W13/tmp4 (2026-09-12)
OUT_SHP = OUT + r"\shp"
OUT_DXF = OUT + r"\dxf"
OUT_IMG = OUT + r"\img"
OUT_RUN = OUT + r"\run"
...
# ---- the area
AREA_SHP = None   # W15: no boundary file; the area is every street of the DXF (AREA_MODE)
...
GROUND_RES_M = 4.0   # W15: 4 m over the whole area (engineer, 2026-09-12); the boundary runs read 2 m
...
STREAM_RES_M = 8.0   # W15: the streams picture is a backdrop, 8 m over the whole
...
STAGE = "A"   # W15: stage A first over the whole network; B after the layout is accepted
...
# ---- W15 (2026-09-12): the whole network
AREA_MODE = "DXF"           # the area is every street the draftsman drew, buffered AREA_BUFFER_M; the settlements are its
                            # dense parts (SETTLEMENT_MIN_HA and up); the 2006 network is a reference only
SETTLEMENT_MIN_HA = 20.0    # a buffered street cluster this big is a settlement (one road-corridor entry each); smaller
                            # clusters and the long inter-settlement roads are the roads outside the settlements
GUIDE_SHP = BASE + r"\Hydraulic\SHP\Main Pipe\Sub Main Pipe guide.shp"   # the engineer's sub main guide for the west
                            # (2026-09-12): streets of the network, forced sub mains toward the main pipe, a join at the foot
GUIDE_SNAP_M = 100.0        # a guide end this close to the main pipe is tied to it with a straight connector
