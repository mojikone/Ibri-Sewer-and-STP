...
Run from ``W11a/report/``::
    ...
    python fig_overview.py F03 F08    # just these
    python fig_overview.py --list     # what exists and what each one says
    python fig_overview.py --facts    # every number, with its artefact, no drawing
...
*   **No guideline number appears on any figure in this module.**  These are scope
    and context figures; nothing here is a compliance check.
...
here and the one every percentage-of-study-area figure is divided by.  The figkit
locator inset is pointed at the same polygon by seeding its documented cache; no
figkit source is modified.
"""
...
def fig01_scope_and_existing() -> Path:
    ...
    km_b = fact("F01", "built gravity sewer in boundary, km",
                round(built.LEN_M.sum() / 1000, 1), "SEWERLINE_IBRI.shp STATUS='Ex'")
    km_p = fact("F01", "proposed gravity sewer in boundary, km",
                round(prop.LEN_M.sum() / 1000, 1), "SEWERLINE_IBRI.shp STATUS='Design'")
    km_fb = fact("F01", "built rising main in boundary, km",
                 round(fmn_b.LEN_M.sum() / 1000, 2), "FORCELINE_IBRI.shp STATUS='Ex'")
    km_fp = fact("F01", "proposed rising main in boundary, km",
                 round(fmn_p.LEN_M.sum() / 1000, 1), "FORCELINE_IBRI.shp STATUS='Design'")
    km_te = fact("F01", "proposed treated-effluent main in boundary, km",
                 round(te.LEN_M.sum() / 1000, 1), "TE_LINE_IBRI.shp (no 'Ex' rows)")
    pct_unbuilt = fact("F01", "share of the sewer dataset never built, %",
                       round(100 * km_p / (km_p + km_b), 1),
                       "SEWERLINE_IBRI.shp STATUS")
    area = fact("F01", "study area, km2", round(study_area_km2(), 1),
                "Hydraulic/SHP/Study area/Project Boundary.shp")
    n_set = fact("F01", "settlements", len(servicing()), "W11a.gpkg [servicing]")
    n_plot = fact("F01", "plots carrying a load record", len(plot_loads()),
    ...
    finish_map(fig, ax, note=note, legend_handles=handles, legend_loc="upper left",
                  ...
                  source=src(
                      ...
                      + CITE_STP + "; " + CITE_CFG))
    return fk.save(fig, "F01_scope_and_existing_assets")
...
def fig02_system_by_settlement() -> Path:
    ...
    pct_off = fact("F02", "load NOT on the central network, %",
                   round(100 * off_q / tot_q, 2), "W11a.gpkg [servicing] Q_ADF_M3D")
    fact("F02", "settlements off the central network", off_n, "W11a.gpkg [servicing]")
    fact("F02", "total ultimate Qadf, m3/d", round(tot_q, 0),
         "W11a.gpkg [servicing] Q_ADF_M3D summed over 187 rows")
    n_prov = fact("F02", "settlements whose decision is PROVISIONAL",
                  int((sv.CONFIDENCE == "provisional").sum()),
                  "W11a.gpkg [servicing] CONFIDENCE")
    n_both = fact("F02", "settlements carrying BOTH options forward", int((sv.BOTH == 1).sum()),
    ...
    finish_map(fig, ax, note=note, legend_handles=handles, legend_loc="upper left",
                  databox=box, source=src(servicing(), boundary(),
                                                     plots_display()))
    return fk.save(fig, "F02_system_by_settlement")
...
def fig03_load_concentration() -> Path:
    ...
    top1 = fact("F03", "largest settlement share of load, %",
                round(100 * sv.loc[0, "Q_ADF_M3D"] / tot, 1),
                "W11a.gpkg [servicing] Q_ADF_M3D")
    top1_name = sv.loc[0, "NAME"]
    top3 = fact("F03", "top-3 settlements share of load, %", round(100 * cum[2], 1),
                "W11a.gpkg [servicing]")
    n90 = fact("F03", "settlements needed to reach 90 % of load",
               int((cum < 0.90).sum()) + 1, "W11a.gpkg [servicing]")
    n_zero = fact("F03", "settlements carrying no load at all",
    ...
    return fk.save(fig, "F03_load_concentration")
...
def fig04_load_density() -> Path:
    ...
    km2_50 = fact("F04", "area holding 50 % of the load, km2",
                  round(n50 * cell_km2, 1), f"W10_plot_loads.gpkg binned to {CELL_M:.0f} m")
    km2_90 = fact("F04", "area holding 90 % of the load, km2",
                  round(n90 * cell_km2, 1), f"W10_plot_loads.gpkg binned to {CELL_M:.0f} m")
    pct50 = fact("F04", "that 50 % as a share of the study area, %",
                 round(100 * km2_50 / area, 1), "against Project Boundary.shp 531.4 km²")
    pct90 = fact("F04", "that 90 % as a share of the study area, %",
                 round(100 * km2_90 / area, 1), "against Project Boundary.shp 531.4 km²")
    n_cells = fact("F04", "loaded cells", int((grid > 0).sum()), "binned plot loads")
    fact("F04", "total load on the grid, m3/d", round(tot, 0),
    ...
    q_lo = tot - q_hi - q_mid
    fact("F04", "load in the top 50 % band, m3/d", round(q_hi, 0), "binned plot loads")
    fact("F04", "load in the tail beyond 90 %, m3/d", round(q_lo, 0), "binned plot loads")
    fact("F04", "cells in the tail beyond 90 %", int((cls == 1).sum()),
         "binned plot loads")
    ...
    return fk.save(fig, "F04_load_density")
...
def fig05_stp_candidates() -> Path:
    ...
    n = len(d)
    fact("F05", "candidates ranked", n, "W10/run/p4_stp_candidates.csv")
    fact("F05", "rank of the existing works", int(ex.RANK), "p4_stp_candidates.csv RANK")
    fact("F05", "free land at the existing works, ha", ex.FREE600_HA,
         "p4_stp_candidates.csv FREE600_HA")
    fact("F05", "free land at the best candidate S1, ha", s1.FREE600_HA,
         "p4_stp_candidates.csv FREE600_HA")
    fact("F05", "nearest dwelling to the existing works, m", ex.D_DWELL_M,
         "p4_stp_candidates.csv D_DWELL_M")
    fact("F05", "nearest dwelling to the southern site, m", so.D_DWELL_M,
         "p4_stp_candidates.csv D_DWELL_M")
    ...
    return fk.save(fig, "F05_stp_candidates")
...
def fig06_stp_compare() -> Path:
    ...
    return fk.save(fig, "F06_stp_site_comparison")
...
def fig07_terrain_and_fall() -> Path:
    ...
    zmin = fact("F07", "lowest ground inside the boundary, m aOD", round(float(z.min()), 1),
                "ibri_blend.tif 5 m, masked to Project Boundary.shp")
    zmax = fact("F07", "highest ground inside the boundary, m aOD", round(float(z.max()), 1),
                "ibri_blend.tif 5 m, masked to Project Boundary.shp")
    p5, p95 = np.percentile(z, [5, 95])
    fact("F07", "5th/95th percentile ground, m aOD", (round(float(p5), 1), round(float(p95), 1)),
         "ibri_blend.tif 5 m, masked to Project Boundary.shp")
    g_hi = fact("F07", "highest trunk chamber ground level, m aOD",
                round(float(tn.GRD_M.max()), 1), "W11a_trunk.gpkg [nodes] GRD_M")
    g_lo = fact("F07", "lowest trunk chamber ground level, m aOD",
                round(float(tn.GRD_M.min()), 1), "W11a_trunk.gpkg [nodes] GRD_M")
    tkm = fact("F07", "trunk length, km", round(float(tr.LEN_M.sum() / 1000), 2),
               "W11a_trunk.gpkg [reaches] LEN_M")
    fall = fact("F07", "ground fall along the trunk, m", round(g_hi - g_lo, 1),
                "W11a_trunk.gpkg [nodes] GRD_M range")
    z_stp = fact("F07", "ground at the existing works, m aOD",
    ...
    no_dem = np.isnan(dem)
    n_none = fact("F07", "frame pixels with no terrain answer in either blend",
    ...
    return fk.save(fig, "F07_terrain_and_fall")
...
def fig08_break_sensitivity() -> Path:
    ...
    fact("F08", "settlements sitting in the 15–18 m band", len(flip),
    ...
    return fk.save(fig, "F08_break_sensitivity")
...
def fig09_load_by_category() -> Path:
    ...
    tot = float(g.q.sum())
    dom_pct = fact("F09", "domestic share of ultimate load, %",
                   round(100 * float(g.loc["domestic", "q"]) / tot, 1),
                   "W10_plot_loads.gpkg CAT / Q_AVG_M3D")
    zero = g[g.q <= 0]
    n_zero = fact("F09", "plots carrying no load at all", int(zero.n.sum()),
                  "W10_plot_loads.gpkg Q_AVG_M3D <= 0")
    n_agri = fact("F09", "agricultural plots carrying no load", int(g.loc["agricultural", "n"]),
                  "W10_plot_loads.gpkg CAT='agricultural'")
    n_ind = fact("F09", "industrial plots carrying no load", int(g.loc["industrial", "n"]),
                 "W10_plot_loads.gpkg CAT='industrial'")
    fact("F09", "total ultimate load, m3/d", round(tot, 0), "W10_plot_loads.gpkg")

    fig, axes = fk.chart_frame(
        title=(f"{dom_pct:.0f} % of the load is domestic — and {n_zero:,} plots are "
               f"modelled as producing nothing"),
        subtitle=(f"Ultimate saturated load by land-use category. The four loaded "
                  f"categories carry all {tot:,.0f} m³/d between them. "
    ...
    return fk.save(fig, "F09_load_by_category")
...
def fig10_existing_coverage() -> Path:
    ...
    pct_q = fact("F10", f"ultimate load within {FRONTAGE_M:.0f} m of a BUILT sewer, %",
                 round(100 * q_near / tot, 1),
                 f"W10_plot_loads.gpkg vs SEWERLINE_IBRI STATUS='Ex', {FRONTAGE_M:.0f} m band")
    pct_n = fact("F10", f"plots within {FRONTAGE_M:.0f} m of a BUILT sewer, %",
                 round(100 * near.mean(), 1), "same")
    fact("F10", "plots already fronted by a built sewer", int(near.sum()),
         "same")
    fact("F10", "load with no built sewer in front of it, m3/d", round(tot - q_near, 0),
         "same")
    km_b = fact("F10", "built gravity sewer in boundary, km",
    ...
    pl.loc[~near].plot(ax=ax, facecolor=fk.C.PLOT_FILL, edgecolor="none", alpha=0.85,
                       zorder=2)
    pl.loc[near].plot(ax=ax, facecolor=fk.C.TRUNK, edgecolor="none", alpha=0.9, zorder=3)
    built.plot(ax=ax, color=fk.C.STATION, lw=0.8, zorder=5)
    boundary().boundary.plot(ax=ax, color=fk.C.BOUNDARY, lw=1.3, ls="--", zorder=8)
    works_markers(ax)
...
def main(argv: list[str]) -> int:
    ...
    if facts_only:
        # compute without drawing: every figure function draws, so run them and
        # then print FACTS -- the point is the table, not the PNGs
        pass
