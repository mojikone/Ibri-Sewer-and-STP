...
class StructureResolver:
    ...
    def _rederive_and_offset(self, net: Network, units):
        ...
        gate_r = self.crit.GATE_SEARCH_M

        def start_chainage(geom):
            """Where the run should start: at the FIRST HOUSE GATE on this street.

            The gate is the plot centroid dropped square onto the street (user 2026-08-20).
            Starting a run at a fixed distance from the junction put the head chamber in
            the middle of nowhere; starting it at the gate puts it where the first house
            actually connects, so no pipe is laid past nothing. Falls back to the fixed
            offset only where no plot faces the street at all."""
            if unit_tree is None:
                return off
            mid = geom.interpolate(0.5, normalized=True)
            near = unit_tree.query_ball_point([mid.x, mid.y],
                                              geom.length / 2.0 + gate_r)
            best = None
            for i in near:
                u = units[i]
                pt = Point(u.x, u.y)
                chn = geom.project(pt)
                if chn <= 0.0 or chn >= geom.length:
                    continue                       # the gate is off the end of this street
                if geom.interpolate(chn).distance(pt) > gate_r:
                    continue                       # too far back to face this street
                if best is None or chn < best:
                    best = chn
            if best is None:
                return off
            return max(0.0, min(best, geom.length - 5.0))
