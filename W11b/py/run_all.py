#!/usr/bin/env python
"""W11b - run the whole pipeline from a cold start, then audit what it produced.
...
PY_DIR = Path(__file__).resolve().parent          # .../W11b/py
W11B = PY_DIR.parent                              # .../W11b
...
class Stage:
    ...
    publishes: Tuple[str, ...] = ()  # the GeoPackage(s) under W11b/shp it must produce
...
PREREQ_DERIVED: List[Tuple[str, Path, str]] = [
    ("the R5 flow-direction grid", W11B / "run" / "terrain" / "R5_d8.tif",
     "python -m w11b.terrain build --grid R5   (hours, ~25 GB - do not rebuild casually)"),
    ("the R5 accumulation grid", W11B / "run" / "terrain" / "R5_acc.tif",
     "python -m w11b.terrain build --grid R5"),
    ("the stream network", W11B / "shp" / "W11b_streams.gpkg",
     "python -m w11b.streams   (about 85 s)"),
]

MODULE_SELFTESTS: List[Tuple[str, float]] = [
    ("w11b.criteria", 60), ("w11b.hydra", 60), ("w11b.contract", 120),
    ("w11b.pumping", 120), ("w11b.hazard", 180), ("w11b.asbuilt", 300),
]
...
def show_plan() -> int:
    ...
    say("\n  Published GeoPackages expected under W11b/shp:")
    for s in STAGES:
...
def audit(extra: Sequence[str]) -> int:
    ...
    say("\n  W11b has no separate auditor module. These checks ARE the auditor: each one\n"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    say(_c("W11b - full pipeline run", "1"))
    say(f"  py      {PY_DIR}")
    say(f"  shp     {W11B / 'shp'}")
