...
and `reaches` - into `W11b/shp/W11b.gpkg`, which is the file `s7_pumps` reads for its
station demands.
...
from w11b import contract as K                                     # noqa: E402
from w11b import hydra as HY                                       # noqa: E402
from w11b.criteria import DEFAULT as CRIT, Criteria, replace       # noqa: E402

STAGE = "s6_levels"
STAGE_VERSION = "W11b-s6_levels-1.0"
STAGE_ORDER = 6

W11B = HERE.parent
CHAMBERS_GPKG = W11B / "shp" / "W11b_chambers.gpkg"
HIER_GPKG = W11B / "shp" / "W11b_hier.gpkg"
FLOWS_GPKG = W11B / "shp" / "W11b_flows.gpkg"
ROADS_GPKG = W11B / "shp" / "W11b_roads.gpkg"
STREAMS_GPKG = W11B / "shp" / "W11b_streams.gpkg"

OUT_GPKG = W11B / "shp" / K.GPKG_NAME               # W11b.gpkg - the CONTRACT file
DIAG_GPKG = W11B / "shp" / "W11b_levels.gpkg"       # this stage's own diagnostics
RUN_DIR = W11B / "run" / "levels"
...
def calibrate_frame(reaches: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    from w11b import asbuilt as AB
...
def publish(layers: Dict[str, object], rec: K.StageRecord) -> None:
    ...
    p = K.publish(cross, "crossings", str(W11B), stage=STAGE, allow_empty=True)
    rec.wrote("crossings", p, len(cross))
    K.assert_crossings_resolve(reaches=reaches, crossings=cross)

    p = K.publish(nodes, "nodes", str(W11B), stage=STAGE)
    rec.wrote("nodes", p, len(nodes))
    p = K.publish(reaches, "reaches", str(W11B), stage=STAGE)
...
def write_kmz(layers: Dict[str, object], rec: K.StageRecord) -> List[str]:
    ...
    out = []
    for view, layer, fn in (("depth", "reaches", "W11b_levels_depth.kmz"),
                            ("diameter", "reaches", "W11b_levels_diameter.kmz"),
                            ("drops", "nodes", "W11b_levels_drops.kmz")):
        gdf = layers[layer]
        if view == "drops":
            ...
            if gdf.empty:
                continue
        try:
            r = P.kmz(gdf, view, str(W11B / "shp" / fn),
                      source=f"{STAGE_VERSION} / {K.CONTRACT_VERSION}")
            out.append(str(W11B / "shp" / fn))
            rec.wrote(f"kmz:{view}", str(W11B / "shp" / fn), len(gdf))
...
def report(layers: Dict[str, object], tables: Dict[str, pd.DataFrame],
    ...
    out = [f"# W11b STAGE 6 - LEVELS AND SIZES", "",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="W11b stage 6 - levels and sizes")
