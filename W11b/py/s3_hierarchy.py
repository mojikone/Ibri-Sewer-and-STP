"""
s3_hierarchy.py - W11b stage 3.  THE HIERARCHY, ON THE ORIENTED TREE.
...
STAGE = "s3_hierarchy"
STAGE_VERSION = "W11b-hierarchy-1.0"
...
OUT_GPKG = os.path.join(W11B, "shp", "W11b_hier.gpkg")
RUN = os.path.join(W11B, "run", "hier")
...
class Hier:
    ...
    def measure(self):
        ...
        self.compare_tier = pd.DataFrame([
            {"BUCKET": "trunk main", "W11b_PCT": 100.0 * km_main / km_all,
            ...
            {"BUCKET": "sub main", "W11b_PCT": 100.0 * tier_km["sub main"] / km_all,
            ...
            {"BUCKET": "lateral + main  (= NAMA's 'lateral' token)",
             "W11b_PCT": 100.0 * nama_lateral / km_all,
        ...
        self.run_stats = pd.DataFrame([
            {"ITEM": "runs", "W11b": float(self.n_runs), "BUILT": float(m_runs["runs_n"])},
            {"ITEM": "run length median, m", "W11b": float(np.median(rl)),
             "BUILT": m_runs["run_between_junctions_median_m"]},
            {"ITEM": "run length p90, m", "W11b": float(np.percentile(rl, 90)),
             "BUILT": m_runs["run_between_junctions_p90_m"]},
            {"ITEM": "run length MAXIMUM, m  (the governing statistic, philosophy sec 4)",
             "W11b": float(rl.max()), "BUILT": float("nan")},
            {"ITEM": "junctions per km", "W11b": float(
                ((np.bincount(self.V[self.keep], minlength=self.n_nodes) >= 2).sum())
                / km_corr), "BUILT": m_runs["junctions_per_km"]},
            {"ITEM": "heads per km", "W11b": float(self.is_head.sum() / km_corr),
    ...
    def manifest(self) -> pd.DataFrame:
        ...
        rows = [
            ...
            ("criteria", CR.CRITERIA_VERSION if hasattr(CR, "CRITERIA_VERSION")
             else "w11b/criteria.py", "", "w11b/criteria.py"),
    ...
    def report(self) -> str:
        ...
        L = []
        A = L.append
        A(f"# W11b stage 3 - the hierarchy, on the oriented tree\n")
        ...
        A(_md(self.uphill, 2) + "\n")
        A("A hierarchy re-labels pipe. It does not tilt ground. The two W11b rows differ "
    ...
    def kmz(self):
        ...
        try:
            ...
            d = gpd.GeoDataFrame(pd.concat([d, tr], ignore_index=True),
                                 crs=d.crs)
            out = os.path.join(W11B, "shp", "W11b_hier_tier.kmz")
