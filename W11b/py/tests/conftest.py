"""W11b test suite - shared fixtures, path setup and markers.
...
cheap ones first:
    ...
    @pytest.mark.published   reads the published GeoPackages under W11b/shp/.  SKIPS, with
...
NO TEST HERE INVENTS A DESIGN NUMBER.  Every threshold is either read from `w11b.criteria`
(which cites its guideline page), read from `w11b.contract` (a structural tolerance that
...
PY_DIR = TESTS_DIR.parent                    # .../W11b/py
W11B_ROOT = PY_DIR.parent                    # .../W11b
REPO_ROOT = W11B_ROOT.parent                 # .../Hydraulic/Claude
SHP_DIR = W11B_ROOT / "shp"
RUN_DIR = W11B_ROOT / "run"

# `w11b` and the stage modules both live in W11b/py. Inserted rather than appended so a
...
GPKGS: Dict[str, str] = {
    "roads": "W11b_roads.gpkg",         # s1_roads.py
    "orient": "W11b_orient.gpkg",       # s2_orient.py build
    "hier": "W11b_hier.gpkg",           # s3_hierarchy.py build
    "chambers": "W11b_chambers.gpkg",   # s4_chambers.py
    "flows": "W11b_flows.gpkg",         # s5_flows.py
    "pumps": "W11b_pumps.gpkg",         # s7_pumps.py
    "streams": "W11b_streams.gpkg",     # w11b.streams
}
...
@pytest.fixture(scope="session")
def contract():
    import w11b.contract as c
    return c


@pytest.fixture(scope="session")
def hydra():
    import w11b.hydra as h
    return h
