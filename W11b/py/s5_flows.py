...
STAGE = "s5_flows"
STAGE_VERSION = "W11b-s5_flows-1.0"
STAGE_ORDER = 5

W11B = HERE.parent
ORIENT_GPKG = W11B / "shp" / "W11b_orient.gpkg"
ROADS_GPKG = W11B / "shp" / "W11b_roads.gpkg"
OUT_GPKG = W11B / "shp" / "W11b_flows.gpkg"
OUT_KMZ = W11B / "shp" / "W11b_flows_peak.kmz"
RUN_DIR = W11B / "run" / "flows"
REPORT_MD = RUN_DIR / "FLOWS.md"

# The plot loads are DATA, produced upstream of W11b and read, never recomputed here.
PLOT_LOADS = W11B.parent / "W10" / "shp" / "W10_plot_loads.gpkg"
...
def asbuilt_benchmark() -> Optional[Dict[str, float]]:
    ...
    try:
        from w11b.asbuilt import AsBuilt
        ab = AsBuilt()
...
def _manifest(arcs, nodes_out, alloc, placed, delivered, lost, net_len_delivered,
    ...
    rows = [
        ...
        r("contract", K.CONTRACT_VERSION, "-", "w11b.contract"),
...
def _report_md(arcs, nodes_out, health, verdict, cons, infil, pfb, bands, roles, outf,
    ...
    L = []
    A = L.append
    A("# W11b stage 5 - flow accumulation")
    A("")
    ...
    A("## Outputs")
    A("")
    A(f"- `{OUT_GPKG.relative_to(W11B.parent)}` - 15 layers")
    A(f"- {kmz_note}")
    A(f"- `{RUN_DIR.relative_to(W11B.parent)}/*.csv` - every table above")
    A(f"- `{REPORT_MD.relative_to(W11B.parent)}` - this file")
    return "\n".join(L)
