...
W11b borrows nothing.  This file imports ``numpy``, ``rasterio`` and its own sibling
``w11b.criteria`` (which imports nothing).  No W8/W10/W11a code is reached, directly or
indirectly.
...
0.  THE CLASS THRESHOLDS ARE NOT DECLARED HERE.  "Which classes are washout ground",
    ...
    ``w11b.criteria`` and this module reads them.  Until 2026-09-03 both files declared
...
--------------------------------------------------------------------------------------
    ...
    from w11b.hazard import HazardGrids, governing, RETURN_PERIODS, HAZARD_FLAGS
...
Author: W11b pipeline.  2026-09-03.
"""
...
try:  # rasterio is the only heavyweight dependency
    ...
    raise ImportError(
        "w11b.hazard needs rasterio.  See _SETUP/ENVIRONMENT.md."
    ) from _exc

# The design basis.  A SIBLING, not a borrow: w11b.criteria imports nothing at all, so
...
# `python W11b/py/w11b/hazard.py` still runs as a bare script.
try:
    from .criteria import Criteria, DEFAULT as CRIT
except ImportError:  # pragma: no cover - direct script execution
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from w11b.criteria import Criteria, DEFAULT as CRIT
...
def _find_hazard_dir() -> Path:
    ...
    Order: ``$W11B_HAZARD_DIR``, then a walk up from this file looking for the project
    tree.  Nothing is guessed silently — if it is not found, we say where we looked.
    """
    env = os.environ.get("W11B_HAZARD_DIR")
    if env:
        p = Path(env)
        if p.is_dir():
            return p
        raise FileNotFoundError(f"W11B_HAZARD_DIR is set to {p!s} but it is not a directory")
    ...
    raise FileNotFoundError(
        "Could not find the flood-hazard grids.  Set $W11B_HAZARD_DIR.  Looked in:\n  "
...
# --- project thresholds: DECLARED IN w11b.criteria, READ HERE -------------------------
#
...
# `criteria.HAZARD_WADI_CLASSES`, and every module in W11b follows.
...
HAZARD_FLAGS: Tuple[str, ...] = (
    ...
    "w11b.criteria AND NOWHERE ELSE; the banner prints the one in force.  Until "
...
class LineHazard:
    ...
    #: it -- the thresholds themselves are declared in ``w11b.criteria``, not here.
    channel_class: int = 0
    scour_class: int = 0
...
class HazardGrids:
    ...
    def __init__(
        ...
        """All three thresholds default to ``w11b.criteria``, which is where they are
        ...
        self.crit = crit
        self.threshold_source = "w11b.criteria"
    ...
    def src(self, rp: int):
        ...
        if rp not in self._src:
            ...
            if s.crs is None or s.crs.to_string() != CRS:
                raise HazardDataError(
                    f"{path.name} is in {s.crs}, expected {CRS}.  Every W11b coordinate "
    ...
    def banner(self) -> str:
        ...
        lines = ["W11b flood hazard - flags that travel with every number below:"]
        ...
        lines.append(f"  * thresholds declared in w11b.criteria "
    ...
    with HazardGrids() as hz:
        ...
        for gone in ("DEFAULT_CHANNEL_CLASS", "DEFAULT_SCOUR_CLASS", "STATION_FREEBOARD_M"):
            if gone in globals() or hasattr(HazardGrids, gone):
                say(f"  FAIL {gone} is back; the thresholds live in w11b.criteria only")
                ok = False
        ...
        if abs(hz.station_freeboard_m - CRIT.PS_FLOOR_ABOVE_FLOOD_M) > 1e-12:
            ...
            ok = False
        say(f"  ok  thresholds come from w11b.criteria: channel H{hz.channel_class}+, "
