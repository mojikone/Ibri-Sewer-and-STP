"""w11b.criteria - THE ONLY PLACE A NUMBER LIVES. W11b owns this file; it imports nothing.
...
WHAT WAS CARRIED FROM W8/W11a, AND WHAT CHANGED
    ...
    changed  (1) ONE wall/bedding allowance. W11a carried WALL_ALLOW = 0.05 in criteria and
             ...
             (5) NEW - the terrain-first block. W11b derives flow direction from the ground
...
CRITERIA_VERSION = "W11b-criteria-1.0"
...
class Criteria:
    """Frozen on purpose. A sensitivity run is a NEW OBJECT, never an edit:
        ...
        from w11b.criteria import DEFAULT, replace
    ...
    # THE ONE WALL/BEDDING ALLOWANCE. There is no second copy of this anywhere in W11b, and
    ...
    # ================================================================= wadi / flood hazard
    #
    # THIS BLOCK IS THE ONLY DECLARATION OF THE FLOOD-GRID THRESHOLDS IN W11b.
    ...
    # `w11b.hazard` now READS these three; it declares none of them.  The measured cost of
    ...
    # ================================================================== TERRAIN-FIRST BLOCK
    # New in W11b, and the reason W11b exists. W11a built its layout on ROAD CONNECTIVITY
    ...
    def invert_depth_min(self, dn: int) -> float:
        ...
        THE ONLY DEFINITION. `cover()` is its exact inverse and nothing else in W11b
    ...
    def ASSUMPTIONS(self) -> Dict[str, Tuple]:
        ...
        return {
            ...
            "HAZARD_WADI_CLASSES": (self.HAZARD_WADI_CLASSES,
                                    ...
                                    "THE ONLY DECLARATION IN W11b: w11b.hazard reads this "
            ...
            "INLET_MIN_DEG_NO_RELAXATION": (self.INLET_MIN_DEG,
                                            ...
                                            "tolerance is NOT carried into W11b: a sharp "
    ...
    def CONFLICTS(self) -> Dict[str, str]:
        """Places where the guidelines contradict themselves or each other, and what W11b
        ...
        return {
            "PC sewer minimum size":
                ...
                "Same document, two numbers. W11b takes OD160 - Table 6 is the later table "
            ...
            "Two K values for one tractive constant":
                ...
                "not an error of ours. W11b works consistently in m3/s with TRACTIVE_K_M3S; "
    ...
    def BENCHMARKS(self) -> Dict[str, Tuple]:
        ...
        return {
            ...
            "UPHILL_SHARE_W11A": (0.425, "share of W11a's 1,731.7 km draining against the "
                                  "ground - 737.7 km. THE DEFECT W11b EXISTS TO FIX. W11b "
    ...
    def RETRACTED(self) -> Dict[str, Tuple]:
        ...
        return {
            ...
            "ASBUILT_KM_188_6": (
                ...
                "W8/docs/LEARNING_FROM_ASBUILT.md - those documents are not W11b's to "
                "edit, but nothing in W11b may cite the figure."),
...
# ======================================================================================
# Self-test. `python -m w11b.criteria` - proves the claims above rather than asserting them.
# ======================================================================================
...
def _self_test(verbose: bool = True) -> None:
    ...
    # (b) the flood-grid thresholds are declared HERE and nowhere else in W11b. hazard.py is
    ...
    try:
        from w11b import hazard as _hz                        # sibling, no cycle
    except Exception:                                          # pragma: no cover
        _hz = None
    if _hz is not None:
        for name in ("DEFAULT_SCOUR_CLASS", "DEFAULT_CHANNEL_CLASS"):
            assert not hasattr(_hz, name), (
                f"w11b.hazard has re-declared {name}. The flood-grid thresholds have ONE "
