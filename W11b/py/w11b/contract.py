"""w11b.contract - the shared data contract. Written before any design stage runs.

W11b owns this file. It imports `w11b.criteria` and `w11b.hydra` and NOTHING from W8, W10
or W11a.
...
1  ONE WALL/BEDDING ALLOWANCE. `min_invert_depth()` and `cover()` below are thin wrappers
   ...
   anywhere in W11b. W11a carried WALL_ALLOW = 0.05 in criteria and AUDITOR_OD_ALLOW_M in
...
----------------------------------------------------------------------------------------
NEW IN W11b: THE TERRAIN-FIRST BLOCK.
...
shafts where NAMA's built network has 37. W11b derives flow direction from the ground first,
...
`_BRAIN/02_DESIGN_CRITERIA.md` and the guideline PDFs via `w11b.criteria` (every number).
NO DESIGN NUMBER IS DEFINED HERE. Values live in `w11b.criteria`; this file names fields,
...
CONTRACT_VERSION = "W11b-contract-1.0"

_HERE = os.path.dirname(os.path.abspath(__file__))       # .../W11b/py/w11b
W11B_ROOT = os.path.dirname(os.path.dirname(_HERE))      # .../W11b
REPO_ROOT = os.path.dirname(W11B_ROOT)                   # .../Hydraulic/Claude
GPKG_NAME = "W11b.gpkg"
...
# ======================================================================================
# Structural constants. NOT design values - those are in w11b.criteria and are cited there.
# ======================================================================================
...
def min_invert_depth(dn: int) -> float:
    ...
    W11b (`criteria.WALL_ALLOW`) and exactly one function that adds it."""
...
BANNED_FIELDS: Dict[str, str] = {
    ...
    "GRADIENT_B": ("the DBF truncation of an 11-character gradient field. If this name is "
                   ...
                   "name - which W11b's 10-character rule exists to make impossible"),
...
REACHES = LayerSpec(
    ...
    fields=(
        ...
        F("GND_FALL", "float", "m", "GROUND fall along this reach, upstream ground minus "
          ...
          "NEW IN W11b and the whole point of it: W11a used the terrain only to check the "
...
CORRIDORS = LayerSpec(
    ...
    fields=(
        ...
        F("GND_FALL", "float", "m", "ground fall end to end. NEW IN W11b: a corridor knows "
...
# ---- the terrain layers. W11b's own, and the reason it exists. -----------------------
#
...
LAYER_ALIASES = {"pipes": "reaches", "manholes": "nodes", "chambers": "nodes",
                 "W11b_pipes": "reaches", "W11b_nodes": "nodes",
                 "W11b_reaches": "reaches", "W11b_manholes": "nodes"}
...
class Network:
    ...
    def outfalls(self) -> List[str]:
        ...
        discharges. This is the only definition of an outfall in W11b."""
    ...
    def ground_fall(self, edge_uid: str) -> float:
        ...
        On the Network rather than in a stage because it is the quantity W11b exists to
...
class Manifest:
    ...
    path = os.path.join(W11B_ROOT, "run", "manifest.json")
...
def mirror_shapefile(gdf, layer_name: str, root: str) -> str:
    ...
    p = os.path.join(root, "shp", f"W11b_{spec.name}.shp")
    os.makedirs(os.path.dirname(p), exist_ok=True)
    gdf.to_file(p)
    return p
...
AUDIT_NEEDS: Dict[str, Dict[str, Tuple[str, ...]]] = {
    ...
    # H17 is W11b's own, and it is the reason W11b exists. Philosophy sec 4 requires the
...
# ======================================================================================
# Self-test. `python -m w11b.contract` - PROVES the invariants bite rather than asserting it.
# ======================================================================================
...
def _self_test(verbose: bool = True) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        sh_n = gpd.read_file(os.path.join(tmp, "shp", "W11b_nodes.shp"))
        sh_r = gpd.read_file(os.path.join(tmp, "shp", "W11b_reaches.shp"))
    ...
    # ------------------------------------------------------------- an empty publish
    _raises(lambda: publish(reaches.iloc[0:0], "reaches", W11B_ROOT, stage="T"),
