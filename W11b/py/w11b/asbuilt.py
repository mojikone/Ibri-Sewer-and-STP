"""
W11b — AS-BUILT TARGETS
=======================

What NAMA actually built in Ibri, measured from the asset shapefiles, turned into
numeric targets a W11b stage can check its own design against.

W11b BORROWS NOTHING.  This module reads only client data and stands alone; it does
not import from W8/py/sewnet, W10/py or W11a/py.  It has no w11b sibling imports
either, so it can be run before the rest of the package exists.
    ...
    python W11b/py/w11b/asbuilt.py            # prints the tables
    python W11b/py/w11b/asbuilt.py --csv DIR  # also writes targets/evidence CSVs
...
# --------------------------------------------------------------------------------------
# PATHS.  Self-contained on purpose (W11b borrows nothing).  Override with W11B_ROOT,
# or by passing paths= to AsBuilt().
# --------------------------------------------------------------------------------------

def _project_root() -> str:
    """.../2621 Ibri Sewer STP — found by walking up from this file."""
    env = os.environ.get("W11B_PROJECT_ROOT")
    if env:
        return env
    here = os.path.abspath(os.path.dirname(__file__))       # .../Hydraulic/Claude/W11b/py/w11b
...
@dataclass
class Target:
    """One checkable number, with the band a W11b design must land in."""
    key: str
    label: str
    value: float | None
    unit: str
...
class AsBuilt:
    ...
    def m_terrain(self) -> dict:
        """Does the built network drain downhill?  The W11b headline test."""
        lv = self.levelled
        L = lv.LEN_M.sum()
        up = lv.GFALL_M < 0
        flat = lv.GFALL_M == 0
    ...
    def m_coverage(self) -> dict:
        ...
        whether W11b reuses it, laid alongside it, or ignores it."""
        pl = self._plot_loads
        cen = pl.geometry.representative_point()
    ...
    def targets(self) -> dict[str, Target]:
        ...
        add(Target("uphill_length_pct",
                   ...
                   note=("THE HEADLINE, and it is NOT zero. NAMA's own network runs uphill on "
                         ...
                         "W11b design above it is worse than anything NAMA built.")))
        ...
        add(Target("cover_below_1p30_pct_len", "length below the 1.30 m minimum cover",
                   ...
                   note=("NAMA BREACHES ITS OWN RULE ON "
                         ...
                         "W11b, not a habit to copy. It is here so nobody cites the as-built as "
        ...
        add(Target("od_below_g203_lateral_min_pct_len",
                   ...
                   note=("DO NOT COPY THE BUILT DIAMETERS. "
                         ...
                         "property connection, not a lateral. W11b's floor is OD 200.")))
        ...
        add(Target("cap_sat_fail_pct",
                   ...
                   note=("a screen, not a model. It answers 'has the 2006 network room' — which "
                         "decides whether W11b reuses it or lays alongside it. The built network "
...
# --------------------------------------------------------------------------------------
# observe_design — turn a W11b design's PUBLISHED layers into the same observables
# --------------------------------------------------------------------------------------
...
# The as-built has THREE tiers.  A W11b design may use more.  The mapping follows
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="W11b as-built targets from NAMA's network")
    ...
    if not a.quiet:
        ...
        print("=" * 100)
        print("W11b AS-BUILT TARGETS   (NAMA Ibri, SEWERLINE_IBRI.shp, measured "
