...
W11b BORROWS NOTHING.  Nothing here is imported from W8/py/sewnet, W10/py or W11a/py.
`w11b.terrain`, `w11b.contract` and `w11b.criteria` are siblings inside W11b and are used as
...
WHAT IT PUBLISHES
----------------------------------------------------------------------------------------
    W11b/shp/W11b_streams.gpkg, layer `streams`, validated against `contract.STREAMS`.
...
CLI
    python -m w11b.streams reconcile     # the measurements -> run/streams/reconcile.json
    python -m w11b.streams build         # publish shp/W11b_streams.gpkg
    python -m w11b.streams verify        # re-read the published layer and check it
    python -m w11b.streams all
"""
...
W11B = T.W11B
RUN = W11B / "run" / "streams"                 # this module's own outputs, nobody else's
GPKG_NAME = "W11b_streams.gpkg"
GPKG = W11B / "shp" / GPKG_NAME
...
def banner() -> str:
    ...
    return "\n".join([
        f"W11b STREAMS | grid {GRID} @ {T.RES_REGIONAL_M:g} m | EPSG:{CRS.split(':')[-1]}",
        "",
        C.tau_banner(),
        "",
...
def ours(clip_to_window: bool = True):
    ...
    if not p.exists():
        raise FileNotFoundError(
            f"{p} does not exist. Build the terrain first: python -m w11b.terrain build")
    g = gpd.read_file(p)
...
def build(mirror_shp: bool = False, validate_it: bool = True) -> str:
    """Publish `W11b/shp/W11b_streams.gpkg`, layer `streams`.
    ...
    if "ORDER" not in g.columns:
        raise RuntimeError(
            ...
            "run. Rebuild it: python -m w11b.terrain build --stages streams --force")
    ...
    if validate_it:
        K.publish(out, "streams", str(W11B), stage=STAGE, gpkg=GPKG_NAME, mirror=mirror_shp)
...
class StreamNet:
    ...
    exists in W11b and this is a facade over it.
    ...
    def load(cls, gpkg: Path | str | None = None, grid: str = GRID,
        ...
        if not p.exists():
            raise FileNotFoundError(
                f"{p} does not exist. Build it: python -m w11b.streams build")
