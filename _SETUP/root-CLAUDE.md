...
- Iterations: work in `Hydraulic/Claude/W#` folders — a rework request means create the next `W#`. **Which folder is live is stated in `Hydraulic/Claude/_BRAIN/00_CURRENT.md` — read it, do not assume.** As of 2026-09-12: **W14** = population, saturation, the flow of every plot and the Concept Design Report R2; **W15** = the pipe-laying engine for the whole network (a complete copy of `W13/tmp4`, which is frozen with `tmp2` and `tmp3` as the test-boundary record; logic `W15/docs/W15_DESIGN_LOGIC.md`); W10–W12 were reverted.
