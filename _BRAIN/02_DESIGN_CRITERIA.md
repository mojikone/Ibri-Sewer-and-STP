...
| ★ **1.5 L/s is NOT in PAM-GUD-203** | Searched the whole PDF (201 pages) on 2026-09-11: no 1.5 L/s anywhere. It is the minimum peak flow of the Mara simplified-sewerage literature (a WC flush), which G203 cites only for the tractive-slope formula. **Use it only as a tagged outside assumption, never as a guideline value**; confirm with NWS | G203 search; Mara, Sleigh & Taylor (2000) as cited p27 |
| ★★ **PROJECT RULE — the two flow cases (engineer, 2026-09-11)** | **Size and capacity on `Q_ULT`** (saturation, 100 % connected — G1-p73 sets coverage at 100 % by the end of the period). **Self-cleansing on the low case `Q_2030` × 0.61** (the TOR's opening year: construction year or 2030; 0.61 = Inception R0 connection ratio reached by 2028), peaked per pipe (Merrimack > 100 properties, Peltier ≤ 100) and fed to both the velocity and the tractive-slope test. Never ultimate × a connection ratio: it mixes 2070 people with a 2028 share and gives new districts 61 % of their saturation flow in 2030, the case where silting happens. Four classes per pipe: **velocity pass; tractive pass (τ is GAP-9, a parameter); early cleansing (low flow, flushing list); fails both with real flow → regrade.** An early-cleansing pipe is flagged for flushing, not upsized. Fields: `W14/shp/PLOTS_load.shp`; handoff `W14/docs/DESIGN_FLOWS_FOR_NETWORK.md` | engineer 2026-09-11; G203 p25–28; G1-p73 |
