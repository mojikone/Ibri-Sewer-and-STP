...
| ★ **Occupancy in use (settled 2026-08-19)** | **OR = 5.0 people per property** (user decision, replaces the 6.0 fallback). **Properties per plot are COUNTED, not assumed**: each electricity account = one property (`IBRI ELE ACCOUNTS.kmz`, 33,970 accounts → `W4/shp/ELE_accounts.shp`). Measured average 1.4 properties/plot; domestic-only 1.46 → 7.28 people/plot, against A7's independent 6.96 from NCSI ÷ built plots. "Additional Account" entries ARE separate dwellings — counting primary only under-predicts by ~19 % | user + accounts data |
| ★ **Load basis (settled 2026-08-19)** | Land-use based, not a blanket per-capita figure. Domestic from counted domestic accounts × OR × per-capita rate; non-domestic identified by account tariff (commercial, government, CRT) rather than the 22 %/14 % fallback ratios. Tab 12 drivers (floor area, pupils, beds, staff) are DERIVED (plot area × cover × storeys, then a density figure) because nobody supplies them — cover known for 17 % of plots, storeys 18 %, so defaults carry the rest. Replace when the treated land-use data arrives | G1-p61 Tab 12 + user |
