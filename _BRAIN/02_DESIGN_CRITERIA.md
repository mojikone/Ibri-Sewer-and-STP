...
| ★ **Two approaches, both mandatory** (read from the PDF 2026-09-11) | *"To ensure the suspended sediment carrying capacity of the sewer, two approaches **shall** be used: 1. Self-cleansing velocity 2. Minimum Tractive force"*; *"Steeper gradient calculated based on self-cleansing velocity and minimum tractive force methodology shall be adopted as minimum pipe gradient."* *"At the head of the sewerage systems, the flow velocity based on the minimum self-cleansing may not be attainable. In these circumstances, the minimum pipe gradient for the sewer shall be calculated based on the hydraulic design approach of minimum tractive force."* | p25–27 §4.2.2.1 |
| ★ **§4.2.6 wording, exact** | *"During early development phases, the actual flow will usually be below the flow of design inducing risk of clogging due to low velocity. The operator should proceed to more frequent inspections and cleansings during this period."* **No flow threshold, no frequency, no duration.** | p28 §4.2.6 |
| ★ **1.5 L/s is NOT in PAM-GUD-203** | Searched the whole PDF (201 pages) on 2026-09-11: no 1.5 L/s anywhere. It is the minimum peak flow of the Mara simplified-sewerage literature (a WC flush), which G203 cites only for the tractive-slope formula. **Not used in this project (engineer 2026-09-11)**: the audit has no low-flow threshold | G203 search; Mara, Sleigh & Taylor (2000) as cited p27 |
| ★★ **PROJECT RULE — the two flow cases and the self-cleansing audit (engineer, 2026-09-11, confirmed)** | **Size and capacity on `Q_ULT`** (saturation, 100 % connected — G1-p73 sets coverage at 100 % by the end of the period). **Self-cleansing on the low case `Q_2030` × 0.61** (the TOR's opening year — construction year or 2030 — at the Inception R0 connection ratio): properties counted **connected** (2030 × 0.61) to choose Merrimack (> 100) or Peltier (≤ 100), **no infiltration**. Never saturation × a ratio. **Three classes per pipe:** (1) **velocity pass** — ≥ 0.75 m/s at the low-case peak; (2) **tractive pass** — the laid gradient ≥ Mara Smin at **τ = 1 Pa**, K = 2.33 × 10⁻⁴ with Q in m³/s, on the true flow (no floor); (3) **needs washing** — everything else: the flushing list (G203 §4.2.6). **No regrade class**: a pipe laid to the guideline gradient that still carries too little flow needs washing, not a steeper pipe. **No 1.5 L/s threshold.** Fields: `W14/shp/PLOTS_load.shp`; handoff `W14/docs/DESIGN_FLOWS_FOR_NETWORK.md` | engineer 2026-09-11; G203 p25–28; G1-p73 |
| ★★ **PROJECT RULE — gradients at the concept stage (engineer, 2026-09-11)** | **Laid at the G203 Table 11 minimum** (tertiary pipes at Table 5), in **steps of 0.05 % (0.5 mm/m) for secondary pipes and 0.025 % (0.25 mm/m) for primary trunks of DN500 and up** — constructable and readable on profiles and maps; a tenth of the minimum (W13 rule 8, superseded) is finer than the 20 mm construction tolerance on large pipes, and a single 0.05 % grid would lay DN900 at 1.00 instead of 0.75 mm/m (250 mm deeper per km). **The tractive force does NOT set any gradient at the concept stage**: it is used only as the audit's second test, at τ = 1 Pa, and applied to the gradients at the preliminary design once NWS confirms τ. **This departs from G203 §4.2.2.1 ("shall"): recorded in report R2 §10.1 for NWS confirmation** | engineer 2026-09-11; G203 p27, p29 |
...
- ★★ **TIER NAMES — the guideline's, not ours (read 2026-09-11, G203 p17, p21).** **Primary** sewage network = the **trunk mains**. **Secondary** sewage network = *"the headers or main sewers usually laid under the streets"*; the **Main Sewer** is secondary. **Tertiary** sewage network = **rider sewers and lateral sewers** (lateral max 45 m), with the property connection sewers often associated. **A guideline "lateral" is the short pipe from the house connection chamber or rider to the street main — NOT a street pipe.** The W8/W13 engine's `TIER` value "lateral" means the street main sewer and clashes with this. Mapping from W13/tmp3 on: `trunk` → **primary**, `sub main` → **secondary header**, `lateral` → **secondary main sewer**; the word "lateral" is reserved for the tertiary pipe. W8 and older outputs keep their old values as the record.
...
| Item | Value | Ref |
|---|---|---|
| ★ Pipelines+chambers in wadis | **AVOID — and the verbs are mandatory.** p30 §4.4.1(a): *"Locating pipelines and associated chambers in wadis or areas subject to washout during heavy storms **must be avoided**."* p33: *"…**shall be avoided**."* **Design rule (user, 2026-09-07): we design to avoid it; where the design finds NO OTHER WAY, we read it as `shall` and the presence is a recorded, justified exception carried to the deliverable — never a silent one.** Running ALONG a wadi and CROSSING one are different questions: crossing has its own procedure (G201 §9.3) and its own cover below | p30 §4.4.1(a), p33 |
| ★ **Cover at a wadi crossing — GRAVITY AND FORCE MAIN ALIKE** | **1.5 m to crown** (against 1.3 m normal). **This is a GUIDELINE value, not a project assumption.** §8.2.4 opens *"**As for gravitational sewer**, the minimum cover should be…"* and then lists 1.3 m without protection, 0.5 m with protection and 1.5 m at a wadi crossing — the first two being exactly the gravity figures, which settles that the list applies to both. *We previously recorded this as "the force-main figure, adopted for gravity by project decision". That was a misreading of the clause's own first four words and it made a cited requirement look weaker than it is.* | **p52 §8.2.4** |
| ★ **The 0.5 m cover is a CONDITIONAL EXCEPTION, not a floor** | p33 §4.6.3: *"The minimum depth for sewer pipes **shall be 1.3 m** to the crown… **If circumstances require installation of a pipe with depth less than 1.3 m above the crown, then concrete protection is required.** The minimum cover above the pipe **and its protection** shall be **0.5 m**."* So 0.5 m buys nothing on its own: it needs (a) a stated circumstance, (b) concrete protection, and (c) the 0.5 m measured **above the protection**, not above the pipe. A reach laid at 0.5 m with no protection detail is simply under cover. **Whether this exception is available AT a wadi crossing is NOT stated by the guideline** — our position is no, because the 1.5 m answers scour and concrete encasement answers external load; recorded as ours, not theirs | p33 §4.6.3 |
| ★ **What counts as "wadi ground"** | `HAZARD_WADI_CLASSES = (4, 5, 6)` of the 50-year grid. These are **AR&R flood-hazard classes** keyed on danger to people and vehicles (class 4 ≈ 1.2 m of water) standing in for the guideline's **washout/SCOUR** criterion. **PROJECT ASSUMPTION**, not a guideline threshold — settle with a scour-depth check | assumption |
...
| ★★ **Maximum cover — 12 m is a HARD LIMIT for now, and the guideline says why it eventually is not** | p33 §4.6.3, verbatim: *"The **recommended** maximum cover for sewer pipes is **approximately 10 – 12 m**. Depths with cover greater than this **shall be investigated with pipe manufacturers**… **Where the cost of excavation becomes prohibitive the Engineer shall incorporate pumping stations into the design**."* So the guideline's real trigger for a station is **EXCAVATION COST**, not depth. **STANDING DECISION (user, 2026-09-07): we are not costing anything yet — that comes when the hydraulic design is ready for cost analysis — so until then 12 m is a HARD LIMIT and nothing goes deeper.** The cost test is the thing that would justify going past it, and we have not run it, so we do not get the licence. **DO NOT read "it is only a recommendation" as permission to exceed it, and DO NOT build exits around it.** The reverted iterations did exactly that: W12 capped at 12 m, then allowed any breach "within 1,000 m of an outfall", and 1,362 chambers came through that one exit with the deepest at 19.98 m. Deleting the exit took it to 3 chambers and 12.79 m. **Revisit only when the cost analysis exists**, and then it is a per-excursion economic question, not a blanket relaxation. | p33 §4.6.3 + standing decision |
...
| Population from plots | Population = plots × avg properties per plot × occupancy rate; OR = Population ÷ Housing Units from NCSI, most recent data, at the geographic scale of the project | G1-p58–59 §7.2.2 |
| ★ **Occupancy in use — PER SETTLEMENT since 2026-09-10 (W14 class v7); the single 5.32 below is the superseded record** | **Live rule:** settlement population 2024 ÷ its domestic properties, floor 4.0, cap 6.12 (highest among settlements of 2,000+ people); a settlement under 1,000 people takes 4.0, one property per home plot and a home share of 0.9 (13 settlements). Ibri 6.07, Ad Dariz 4.86, Al Araqi 4.34. Table `W14/analysis/occupancy_by_settlement.csv`; every plot carries its settlement's rate as `OR_S`. **Superseded (2026-08-30):** OR = 5.32 people per domestic property, measured: 2024 settlement population ÷ domestic properties, both clipped to the same 25 settlements (`Towns.shp` ∩ `IBRI ELE ACCOUNTS.kmz`). Ibri town alone = 6.21. The earlier 5.0 was a placeholder, never a decision. **Properties per plot are COUNTED, not assumed**: 1.456 domestic properties per matched plot. "Additional Account" entries ARE separate dwellings (6,344 = 28 % of domestic) — counting primary only under-predicts by ~19 %. Four settlements are unusable pending boundary review (Sayh Al Masarrat 1.07, Al Jahli 1.30, Al Akheedar 1.64, Satwah 10.12), and the GHS-POP cross-check adds a fifth, **Al Qali** (GHS/NCSI 2.70). GHS finds 1.9-2.6x more people than NCSI in the first three, so their diagnosis is NCSI attribution, not the property count; Satwah is the reverse (GHS agrees with NCSI) so it is a property-count problem. **GHS is a cross-check only and never a load input** - see `W9/analysis/W9_ghs_check.md`. Recompute when the clean plot layer lands | `W9/analysis/W9_ele_landuse.md` |
| ★ **OR validity check** | The literal GUD-201 check is impossible — R0 publishes LPCD 163.5 but never the consumption or customer counts behind it. Equivalent coverage test passes: settlements hold **63.4 %** of wilayat population; at OR 5.32 the wilayat implies 34,504 domestic properties against 22,588 held = **65.5 %**. Agreement within 2.0 points ⇒ 5.32 is a measurement, not a coverage artefact. **Deviation to declare**: G1-p58 defines OR = Population ÷ *Housing Units* from NCSI; settlement-level housing units are not published, so properties are counted from electricity accounts instead | G1-p58 + `W9/docs/INFERENCE_REGISTER.md` |
| ★★ **LOAD BASIS — LOCKED 2026-08-30 (supersedes the 2026-08-19 wording)** | **Tier A ratios set the VOLUME; land use sets the PLACEMENT.** The two axes do not overlap, so this is not tier-mixing and does not double count. (a) Domestic = counted domestic accounts × OR × LPCD. (b) Non-domestic and governmental volume = **+22 % and +14 % of domestic** (Tab 11, Adh Dhahirah). (c) That volume is landed on commercial and government plots identified by account tariff, **not** smeared across population. (d) **Table 12 is NOT used** — it is priced in pupils, beds, employees and floor area, none of which were supplied, so the §7.3.2 "shall" condition is unmet in substance. Adopt Table 12 the day real quantities arrive; do not derive its drivers from plot area × cover × storeys. **Never apply Table 12 and the ratios together.** | G1-p59–61 §7.3 + `W9/docs/INFERENCE_REGISTER.md` |
| ★ **Additive, outside the ratios** | Four streams are excluded from the 22 %/14 % by the guideline itself and are added separately, which is NOT double counting: identified projects such as the industrial estate (*"determined on a case-by-case basis"*, G1-p59); special consumption — labour camps, high-water industry, developer-supplied (§7.3.4); blue and yellow tankers; private wells and other non-network abstraction (§7.4, binding) | G1-p59, p61, p70 |
| ★ **Agricultural plots (settled 2026-08-30)** | An agricultural meter is an **irrigation pump and generates no wastewater**. Evidence: on 172 of 319 agricultural plots NAMA bills a household tariff *alongside* the farm tariff — dwelling and pump are metered separately (428 domestic meters on those plots, 2.49 per plot against 1.46 generally). Rule, not fixed numbers: **an agricultural meter on a plot that already carries a domestic meter adds nothing; an agricultural-only plot carries no dwelling.** Excludes 147 plots ≈ 782 people ≈ 0.67 % of settlement population. Recompute after the clean plot layer arrives | `W9/docs/INFERENCE_REGISTER.md` I-4, I-5 |
| ★ **CRT is unresolved** | 499 accounts on the Cost Reflective Tariff — a **consumption-threshold class, not a land use**. A mall, factory, hotel or large government building all land in it, so tariff alone cannot classify them, and they are the largest consumers. The industrial estate is **not identifiable** in the dataset (one Industrial account; largest CRT clusters are Ibri's commercial core). Resolve against the plot layer or by site check before any ratio is applied | `W9/analysis/W9_ele_landuse.md` |
| ★★ **Accuracy is DIRECTIONAL** | One population figure cannot serve all three uses — the safe direction reverses. **Pipe sizing** (peak flow, distribution): under-estimate surcharges. **STP capacity and staging** (total, growth curve): under-estimate undersizes the plant. **Self-cleansing and the early washing schedule** (MINIMUM early-year flow): **over-estimate is the dangerous one** — pipes assumed to scour themselves while actually silting, and maintenance told to stop washing too soon. Size capacity on the high case, set the wash schedule on the low case, and carry the connection ratio (0.51 in 2024 → 0.61 by 2028, R0) explicitly in the third | G1-p72 §7.4.4 + R0 |
