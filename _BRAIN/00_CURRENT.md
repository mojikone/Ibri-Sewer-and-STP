...
settlement as one sub-network reaching NAMA's corridor to the STP by its own streets, 3 short
links to the main pipe (no road between, under 150 m), no sink (basins drain over their rim
with the extra depth marked), 11 islands; sub-mains as spines, heads at the first gate, 38
contiguous catchments, no loops, one outlet per node, checked in QGIS (group `Claude W13 A`). **The drawing
