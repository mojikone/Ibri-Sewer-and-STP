# What is current, and what is not — checked 2026-09-14

**2026-09-17: the Concept Design Report R3 is built in `W16/report/R3/` (107 pages after the engineer's review the same day: numbered by chapter 1 to 8 with sections 1.1 and 1.1.1 instead of Parts, captions "Figure 2.", Section 1.5.1 one table of the 15 items saying decision requested or adopted, Figure 2 on the boundaries as received, eight charts added in `charts_r3.py`, pages numbered i to xi over the front matter and 1 to 95 from Chapter 1, none on the cover; **Section 1.2 describes the study area**: topography and drainage, climate and wind roses from the public NASA POWER record, roads, flood hazard from the Oman Flood Mapping project of MAFWR, six maps `RPT A01…A06` and four charts, data in `W16/analysis/study_area/`; **the main pipe is not client data and is not shown on client maps**; no meteorological station record has been supplied, **GAP-24**, so the odour dispersion modelling waits for it): the Design Basis Report carried in full, the seven decisions still open after the meeting of 16 September, one tractive-force chart in Section 3.3.2, NWS's 4 % maximum at head pipes noted. R2 stays in `W14/report/R2/` as issued; the register `W16/docs/CHANGES_FOR_CONCEPT_R3.md` is applied.** The deck as presented on 16 September, with the progress section the team added, is the engineer's copy `W16/report_basis/deck/Ibri_Design_Basis_Meeting_2026-09-16 - 2.pptx`; the final file sits on the W: drive under `06-WORKING/04-Hydraulic/8-Meetings/01 Meeting 2026-09-16/Reports/`. No decision was taken at the meeting. **2026-09-14: W16 is the report folder, and the Design Basis Report R0 is built for approval ahead of the concept design.** `W16/` copies W14's scripts, analysis, docs and figures; **the loads are unchanged and stay frozen in `W14/shp`**, read from there. `W16/report_basis/R0/Ibri_Design_Basis_Report_R0.docx` + `.pdf` (49 pages, `build.py`) puts before NWS the settlement boundaries (received and redrawn), the people, the use of each plot, the water and sewage rates, the growth **with and without the overflow**, the flow every element is designed for, the plant flows and loads and the treated effluent, every adopted value in one table, seven data requests and **nineteen decisions** (`report_basis/decisions.py`, one list for the front summary and the register). Conventions set by the engineer and now standing for every report (`W16/report/doc.py`): the firm's template (cover, header, footer, Century Gothic navy headings, no indent), Word-native `SEQ` captions, symbol lines under equations instead of a table, navy-header tables, satellite at 50 %, coloured land use without outlines, **no worked examples**, plain words, a decisions register. The maps are layouts kept in `stp2.qgz` (`RPT B01_boundaries` … `RPT B05_free_meters`, `RPT B05 free-meter panels (atlas)`, group `Claude W16 basis`; the `W2 M1 Study Area` layout imported from the old project). New figures: the saturation flow map with the four-step plant recipe, the 126 free meters with 68 zoom panels (`make_panels.py`, `compose_panels.py`), the self-cleansing curve full and minimal (`charts_basis.py`). **Revised the same day on the engineer's 22 Word comments and 16 chat items** (42 pages): only what the guideline does not settle is put to approval, **7 decisions** (boundaries, occupancy, plot use, overflow, **horizon 2055 or 2070**, growth beyond 2050, the concept gradient rule) and **8 values adopted for information**; "property" not "dwelling", "calculated", STP not "works", Q special, planted not grove, the 126 meters left out with nine example panels in the section, the treatment plant's plot reclassed to government (`facts_w14.PLOT_OVERRIDES`), the flow map labelled for 2055 and 2070 with leaders, five-year tables in the body, no plot-layer delivery mention. Then rebuilt at the wider width (1.5 cm side margins, tables and header and footer at the full 18 cm, grey footer with a centred page number; 40 pages). The commented draft: `W16/report_basis/R0/review/`. **The meeting deck** for 16 September: `W16/report_basis/deck/Ibri_Design_Basis_Meeting_2026-09-16.pptx` (17 slides on the firm's kick-off master, one per decision, a category tab and a page number on every slide, native icons placed through PowerPoint; `build_deck.py`, assets by `make_assets.py`). **The data package for NWS** (2026-09-15): `W16/deliverables/Ibri_Design_Basis_Data_R0.zip`, built by `make_deliverables.py` from the frozen W14 layers: boundaries, settlements, meters, plots and identified projects, each as KMZ and shapefile, the tables as Excel. **The meeting folder** `W16/Meeting 2026-09-16/` (not in git) holds every latest layer, the reports, the tables, the delivered zip and a self-contained QGIS project styled as stp2 and the report maps; rebuilt by `W16/meeting/make_meeting_folder.py`. The QGIS relay hung after the panel render; `W16/report_basis/qgis_direct.py` talks to the plugin's socket directly and made the map exports. **What the concept report's R3 must carry: `W16/docs/CHANGES_FOR_CONCEPT_R3.md`** (wording fixes from the 13 September Q&A too: "peak daily flow" is the peak hourly flow; the 36 and 23 named; "on dwellings"; the treated effluent network; "trade" against "treated" effluent). W15 stays the engine.
...
**2026-09-12: the stages are renamed in the engineer's words, and every earlier "Stage C" below means B and C together.** **A** = the layout, all the engine runs today, run alone per area so the layout is reviewed first. **B** = the hydraulic design of each subnetwork: the real lay in three bands, one gradient between chambers, drops, chambers and Table 12 spacing, the capacity check at the laid gradient, the subnetwork's pumps as candidates, its crossings; run with A (`STAGE = "AB"` in the config) or after it, gated on reproducing A's layout. **C** = the whole network once every area has A and B: the main pipe sized and laid, the joins floored on it, the works connection and the inlet at 323.0 m, the pumps of the whole (a B candidate may move, merge or go), the crossings of the main pipe and the trunks. Defined in `W13/tmp4/docs/W13_TMP4_DESIGN_LOGIC.md`. B is not built yet.
...
**2026-09-12 (later): every bend keeps its chamber.** The engineer's ruling on the drawing: a corner over 5° is never absorbed by a chamber 10 m away, two consecutive corners both stand, a head within 10 m of a bend moves onto it, a head pipe left under 10 m is removed and listed. Left on the boundary: 9 close pairs (sharp corner next to a junction), 28 offset heads, 10 removed head pipes. Every run's line now starts and ends on its chambers (stage A's noding had left up to 0.9 m); a corner within 10 m of a junction chamber is that chamber's; the pipes as laid are `W13/tmp4/shp/W13_B_pipes.shp`, the layer to check chambers against (stage A's pipes are the layout). Left: 2 junctions 3.6 and 4.6 m apart in the draftsman's lines. **Pumps as candidates are built (rule 16)**: C07 pumps 10.9 l/s through 1.58 km of DN150 into the west's header (2.7 kW, under 1.0 m/s, longer than 1,000 m: the engineer's calls), the two islands unrouted; `W13/tmp4/shp/W13_B_pumps.shp`, `W13_B_pmains.shp`. Next in B: the SewerGEMS export on request; then stage C.
...
**2026-09-12 (night, latest): the whole network in Google Earth.** `W15/kmz/W15_A_network.kmz` from `W15/py/export_kmz.py`: a folder per convergence group and per subnetwork, one colour each with neighbours apart, sub mains thick, main pipe yellow, guide magenta, every outlet in a Points layer mirroring group and subnetwork, and three overlays off until ticked (flow arrows, pipes deeper than 12 m, group outlines); `W15_A_network_nolabels.kmz` the same without point labels.
