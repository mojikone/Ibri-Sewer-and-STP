...
| **W4 (current)** | **Test-boundary network design pipeline.** Phase A done (2026-08-18): SWNETWROK storm repo cloned to `_REFERENCE/` and autopsied (keep: noding → outlet-directed spanning tree → top-down invert solver → audit → SHP/DXF skeleton; it computes NO flows/sizes — hydraulic layer is all-new), inputs inspected (551 ha test area, 98 km roads noded+connected, 2,825 plots 2217B/522P/86A, terrain VRT clean, boundary polygon needs make_valid, class_v4 needs utf-8), criteria digest + SewerGEMS ModelBuilder schema researched (`W4/docs/phaseA/`). **Design plan `W4/docs/PLAN.md` awaiting user review** — 9-stage pipeline (prep → topology → manholes/tertiary → loads → sizing ⇄ inverts → audit → exports), Figma flowchart in docs/img. Open decisions: test outfall location, PF Merrimack-vs-Peltier default. After W4: three concept options, SewerGEMS seed, F2 georeferencing, capacity rerun with A6 farm correction |
