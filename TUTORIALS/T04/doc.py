"""Document furniture for T04 (from T03 Rev 01 with the R2 layout fixes) - styles, headings, tables, captions, TOC.
...
def h(d, level, text, page_break=False):
    ...
    if page_break:
        # the break belongs to the heading: a separate break paragraph lands on a
        # blank page whenever the previous page is already full
...
def _mark_size(par, font):
    """Paragraph-mark size sets the line height of an empty cell; without it the row doubles."""
...
def table(d, headers, rows, widths=None, font=9, header_fill="1F3B63",
          align_right=None, keep_together=None):
    """keep_together: None = tables of 16 rows or fewer never break; a longer table
    may break but never leaves its last two rows alone on a page."""
    ...
    for row in (t.rows[:-1] if keep_together else t.rows[-3:-1]):
